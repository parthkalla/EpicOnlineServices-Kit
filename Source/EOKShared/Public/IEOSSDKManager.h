...
DECLARE_MULTICAST_DELEGATE_OneParam(FEOKSDKManagerOnPreInitializeSDK, EOS_InitializeOptions& Options);
DECLARE_MULTICAST_DELEGATE_TwoParams(FEOKSDKManagerOnDefaultPlatformConfigNameChanged, const FString& NewName, const FString& OldName);
DECLARE_MULTICAST_DELEGATE_TwoParams(FEOKSDKManagerOnPreCreateNamedPlatform, const FEOSSDKPlatformConfig& Config, EOS_Platform_Options& Options);
DECLARE_MULTICAST_DELEGATE_OneParam(FEOKSDKManagerOnPreCreatePlatform, EOS_Platform_Options& Options);

class IEOKPlatformHandle
{
public:
	IEOKPlatformHandle(EOS_HPlatform InPlatformHandle) : PlatformHandle(InPlatformHandle) {}
	virtual ~IEOKPlatformHandle() = default;
...
using IEOKPlatformHandlePtr = TSharedPtr<IEOKPlatformHandle, ESPMode::ThreadSafe>;
using IEOKPlatformHandleWeakPtr = TWeakPtr<IEOKPlatformHandle, ESPMode::ThreadSafe>;
...
public:
	 ...
	 */
	virtual IEOKPlatformHandlePtr CreatePlatform(const FString& PlatformConfigName, FName InstanceName = NAME_None) = 0;

	/** Create a platform handle using EOSSDK options directly. */
	virtual IEOKPlatformHandlePtr CreatePlatform(EOS_Platform_Options& PlatformOptions) = 0;
	...
	FEOKSDKManagerOnPreInitializeSDK OnPreInitializeSDK;
	FEOKSDKManagerOnDefaultPlatformConfigNameChanged OnDefaultPlatformConfigNameChanged;
	FEOKSDKManagerOnPreCreateNamedPlatform OnPreCreateNamedPlatform;
	FEOKSDKManagerOnPreCreatePlatform OnPreCreatePlatform;
};
