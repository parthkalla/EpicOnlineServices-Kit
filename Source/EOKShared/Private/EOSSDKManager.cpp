...
namespace
{
	...
	{
#define EOSLOG(Level) UE_LOG(LogEOKSDK, Level, TEXT("%s: %s"), UTF8_TO_TCHAR(Message->Category), *MessageStr)
...
#undef EOSLOG
	...
	static void* GetSdkDllHandle()
	{
		...
		FString PluginModuleName = TEXT("EpicOnlineServicesKitV2");
		...
		if (Plugin.IsValid())
		{
			...
			FString SecondBinaryPath = FPaths::Combine(*PluginDirectory, TEXT("Source"), TEXT("ThirdParty"), TEXT("EOKSDK"), TEXT("Bin"), TEXT(EOSSDK_RUNTIME_LIBRARY_NAME));
			// Check if the file exists before attempting to load
			if (FPaths::FileExists(BinaryPath))
			{
				UE_LOG(LogEOKSDK, Log, TEXT("Loading EOS SDK from plugin binaries: %s."), *BinaryPath);
				Result = FPlatformProcess::GetDllHandle(*BinaryPath);
			}
			else if (FPaths::FileExists(SecondBinaryPath))
			{
				UE_LOG(LogEOKSDK, Log, TEXT("Loading EOS SDK from main plugin binaries: %s."), *SecondBinaryPath);
				Result = FPlatformProcess::GetDllHandle(*SecondBinaryPath);
			}
			else
			{
				UE_LOG(LogEOKSDK, Log, TEXT("EOS SDK binary not found: %s. Now project binaries will be looked up."), *BinaryPath);
			}
		}
		else
		{
			UE_LOG(LogEOKSDK, Log, TEXT("Plugin not found: %s"), *PluginModuleName);
		}
		...
		if(!Result)
		{
			...
			{
				UE_LOG(LogEOKSDK, Log, TEXT("Loading EOS SDK from project binaries: %s"), *ProjectBinaryPath);
				Result = FPlatformProcess::GetDllHandle(*ProjectBinaryPath);
			}
			else
			{
				UE_LOG(LogEOKSDK, Log, TEXT("Unable to find EOS SDK in project binaries: %s"), *ProjectBinaryPath);
			}
		}

		if (!Result)
		{
			UE_LOG(LogEOKSDK, Log, TEXT("Loading EOS SDK from engine binaries. This may not be good if you are using a older version of the SDK."));
		...
		if (!Result)
		{
			UE_LOG(LogEOKSDK, Warning, TEXT("Loading EOS SDK from system path"));
...
// with the call in FEOKSDKManager::FEOKSDKManager, and will only speed it up, even if it's not complete before
...
FEOKSDKManager::FEOKSDKManager()
{
	...
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("Unable to load EOSSDK dynamic library"));
		return;
	}
#endif
}
...
EOS_EResult FEOKSDKManager::Initialize()
{
#if EOSSDK_RUNTIME_LOAD_REQUIRED
	if (SDKHandle == nullptr)
	{
		UE_LOG(LogEOKSDK, Log, TEXT("FEOKSDKManager::Initialize failed, SDKHandle=nullptr"));
	...
	{
		UE_LOG(LogEOKSDK, Log, TEXT("Initializing EOSSDK Version:%s"), UTF8_TO_TCHAR(EOS_GetVersion()));
		...
		static_assert(EOS_INITIALIZE_API_LATEST == 5, "EOS_InitializeOptions updated, check new fields");
		...
		if (EosResult == EOS_EResult::EOS_Success || EosResult == EOS_EResult::EOS_AlreadyConfigured)
		{
			...
			FCoreDelegates::TSOnConfigSectionsChanged().AddRaw(this, &FEOKSDKManager::OnConfigSectionsChanged);
#elif ENGINE_MAJOR_VERSION ==5 && ENGINE_MINOR_VERSION == 1
			FCoreDelegates::OnConfigSectionsChanged.AddRaw(this, &FEOKSDKManager::OnConfigSectionsChanged);
#endif
...
#if !NO_LOGGING
			FCoreDelegates::OnLogVerbosityChanged.AddRaw(this, &FEOKSDKManager::OnLogVerbosityChanged);

			EosResult = EOS_Logging_SetCallback(&EOSLogMessageReceived);
			if (EosResult != EOS_EResult::EOS_Success)
			{
				UE_LOG(LogEOKSDK, Warning, TEXT("EOS_Logging_SetCallback failed error:%s"), *EOK_LexToString(EosResult));
			}

			EosResult = EOS_Logging_SetLogLevel(EOS_ELogCategory::EOS_LC_ALL_CATEGORIES, ConvertLogLevel(LogEOKSDK.GetVerbosity()));
			if (EosResult != EOS_EResult::EOS_Success)
			{
				UE_LOG(LogEOKSDK, Warning, TEXT("EOS_Logging_SetLogLevel failed Verbosity=%s error=[%s]"), ToString(LogEOKSDK.GetVerbosity()), *EOK_LexToString(EosResult));
			}
#endif // !NO_LOGGING
		}
		else
		{
			UE_LOG(LogEOKSDK, Warning, TEXT("EOS_Initialize failed error:%s"), *EOK_LexToString(EosResult));
		}
...
const FEOSSDKPlatformConfig* FEOKSDKManager::GetPlatformConfig(const FString& PlatformConfigName, bool bLoadIfMissing)
{
	if (PlatformConfigName.IsEmpty())
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("Platform name can't be empty"));
		return nullptr;
	}
	...
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("Could not find platform config: %s"), *PlatformConfigName);
		return nullptr;
	}
	...
	UE_LOG(LogEOKSDK, Verbose, TEXT("Loaded platform config: %s"), *PlatformConfigName);
	return PlatformConfig;
}

bool FEOKSDKManager::AddPlatformConfig(const FEOSSDKPlatformConfig& PlatformConfig)
{
	...
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("Platform config already exists: %s"), *PlatformConfig.Name);
		return false;
	}

	PlatformConfigs.Emplace(PlatformConfig.Name, PlatformConfig);
	UE_LOG(LogEOKSDK, Verbose, TEXT("Added platform config: %s"), *PlatformConfig.Name);
	return true;
}

const FString& FEOKSDKManager::GetDefaultPlatformConfigName()
{
...
void FEOKSDKManager::SetDefaultPlatformConfigName(const FString& PlatformConfigName)
{
	if (DefaultPlatformConfigName != PlatformConfigName)
	{
		UE_LOG(LogEOKSDK, Verbose, TEXT("Default platform name changed: New=%s Old=%s"), *PlatformConfigName, *DefaultPlatformConfigName);
...
IEOKPlatformHandlePtr FEOKSDKManager::CreatePlatform(const FString& PlatformConfigName, FName InstanceName)
{
	if (PlatformConfigName.IsEmpty())
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("Platform name can't be empty"));
		return IEOKPlatformHandlePtr();
	}
	...
	TMap<FName, IEOKPlatformHandleWeakPtr>* PlatformMap = PlatformHandles.Find(PlatformConfigName);
	if (PlatformMap)
	{
		IEOKPlatformHandleWeakPtr* WeakPlatformHandle = PlatformMap->Find(InstanceName);
		if (WeakPlatformHandle)
		{
			if (IEOKPlatformHandlePtr Pinned = WeakPlatformHandle->Pin())
			{
				UE_LOG(LogEOKSDK, Verbose, TEXT("Found existing platform handle: PlatformConfigName=%s InstanceName=%s"), *PlatformConfigName, *InstanceName.ToString());
				return Pinned;
			}

			UE_LOG(LogEOKSDK, Verbose, TEXT("Removing stale platform handle pointer: PlatformConfigName=%s InstanceName=%s"), *PlatformConfigName, *InstanceName.ToString());
	...
	IEOKPlatformHandlePtr PlatformHandle = CreatePlatform(*PlatformConfig, PlatformOptions);
	if (PlatformHandle.IsValid())
	{
		UE_LOG(LogEOKSDK, Verbose, TEXT("Created platform handle: PlatformConfigName=%s InstanceName=%s"), *PlatformConfigName, *InstanceName.ToString());
...
IEOKPlatformHandlePtr FEOKSDKManager::CreatePlatform(const FEOSSDKPlatformConfig& PlatformConfig, EOS_Platform_Options& PlatformOptions)
{
	OnPreCreateNamedPlatform.Broadcast(PlatformConfig, PlatformOptions);
	return CreatePlatform(PlatformOptions);
}

IEOKPlatformHandlePtr FEOKSDKManager::CreatePlatform(EOS_Platform_Options& PlatformOptions)
{
	IEOKPlatformHandlePtr SharedPlatform;
	...
	if (IsInitialized())
	{
		...
		if (PlatformHandle)
		{
			...
			SharedPlatform = MakeShared<FEOKPlatformHandle, ESPMode::ThreadSafe>(*this, PlatformHandle);
			SetupTicker();
		...
		{
			UE_LOG(LogEOKSDK, Warning, TEXT("FEOKSDKManager::CreatePlatform failed. EosPlatformHandle=nullptr"));
		}
	}
	else
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("FEOKSDKManager::CreatePlatform failed. SDK not initialized"));
	}

	return SharedPlatform;
}

void FEOKSDKManager::OnConfigSectionsChanged(const FString& IniFilename, const TSet<FString>& SectionNames)
{
...
void FEOKSDKManager::LoadConfig()
{
...
void FEOKSDKManager::SetupTicker()
{
...
#if ENGINE_MAJOR_VERSION == 5
		TickerHandle = FTSTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateRaw(this, &FEOKSDKManager::Tick), TickIntervalSeconds);
#else
		TickerHandle = FTicker::GetCoreTicker().AddTicker(FTickerDelegate::CreateRaw(this, &FEOKSDKManager::Tick), TickIntervalSeconds);
#endif
	}
}

bool FEOKSDKManager::Tick(float)
{
	...
	if (ActivePlatforms.Num())
	{
		...
		for (EOS_HPlatform PlatformHandle : PlatformsToTick)
		{
			...
			QUICK_SCOPE_CYCLE_COUNTER(FEOKSDKManager_Tick);
...
void FEOKSDKManager::OnApplicationStatusChanged(EOS_EApplicationStatus ApplicationStatus)
{
	UE_LOG(LogEOKSDK, Log, TEXT("OnApplicationStatusChanged [%s] -> [%s]"), EOK_LexToString(CachedApplicationStatus), EOK_LexToString(ApplicationStatus));
...
void FEOKSDKManager::OnNetworkStatusChanged(EOS_ENetworkStatus NetworkStatus)
{
	UE_LOG(LogEOKSDK, Log, TEXT("OnNetworkStatusChanged [%s] -> [%s]"), EOK_LexToString(CachedNetworkStatus), EOK_LexToString(NetworkStatus));
...
void FEOKSDKManager::OnLogVerbosityChanged(const FLogCategoryName& CategoryName, ELogVerbosity::Type OldVerbosity, ELogVerbosity::Type NewVerbosity)
{
#if !NO_LOGGING
	if (IsInitialized() &&
		CategoryName == LogEOKSDK.GetCategoryName())
	{
		const EOS_EResult EosResult = EOS_Logging_SetLogLevel(EOS_ELogCategory::EOS_LC_ALL_CATEGORIES, ConvertLogLevel(NewVerbosity));
		if (EosResult != EOS_EResult::EOS_Success)
		{
			UE_LOG(LogEOKSDK, Warning, TEXT("EOS_Logging_SetLogLevel failed Verbosity=%s error=[%s]"), ToString(NewVerbosity), *EOK_LexToString(EosResult));
		}
	}
#endif // !NO_LOGGING
}

FString FEOKSDKManager::GetProductName() const
{
	FString ProductName;
...
FString FEOKSDKManager::GetProductVersion() const
{
	return FApp::GetBuildVersion();
}

FString FEOKSDKManager::GetCacheDirBase() const
{
	return FPlatformProcess::UserDir();
}

FString FEOKSDKManager::GetOverrideCountryCode(const EOS_HPlatform Platform) const
{
	FString Result;
...
FString FEOKSDKManager::GetOverrideLocaleCode(const EOS_HPlatform Platform) const
{
	FString Result;
...
void FEOKSDKManager::ReleasePlatform(EOS_HPlatform PlatformHandle)
{
...
void FEOKSDKManager::ReleaseReleasedPlatforms()
{
...
void FEOKSDKManager::Shutdown()
{
	if (IsInitialized())
	{
		...
		if (ActivePlatforms.Num() > 0)
		{
			UE_LOG(LogEOKSDK, Warning, TEXT("FEOKSDKManager::Shutdown Releasing %d remaining platforms"), ActivePlatforms.Num());
		...
		UE_LOG(LogEOKSDK, Log, TEXT("FEOKSDKManager::Shutdown EOS_Shutdown Result=[%s]"), *EOK_LexToString(Result));
...
EOS_EResult FEOKSDKManager::EOSInitialize(EOS_InitializeOptions& Options)
{
...
bool FEOKSDKManager::Exec(class UWorld* InWorld, const TCHAR* Cmd, FOutputDevice& Ar)
{
	...
	{
		UE_LOG(LogEOKSDK, Warning, TEXT("Unknown exec command: %s]"), Cmd);
	}

	return true;
}

#define UE_LOG_EOSSDK_INFO(Format, ...) UE_LOG(LogEOKSDK, Log, TEXT("Info: %*s" Format), Indent * 2, TEXT(""), ##__VA_ARGS__)

void FEOKSDKManager::LogInfo(int32 Indent) const
{
...
void FEOKSDKManager::LogPlatformInfo(const EOS_HPlatform Platform, int32 Indent) const
{
	UE_LOG_EOSSDK_INFO("ApplicationStatus=%s", EOK_LexToString(EOS_Platform_GetApplicationStatus(Platform)));
	UE_LOG_EOSSDK_INFO("NetworkStatus=%s", EOK_LexToString(EOS_Platform_GetNetworkStatus(Platform)));
	...
	if (Result == EOS_EResult::EOS_Success)
	{
		UE_LOG_EOSSDK_INFO("DesktopCrossplayStatusInfo Status=%s ServiceInitResult=%d",
			EOK_LexToString(GetDesktopCrossplayStatusInfo.Status),
			GetDesktopCrossplayStatusInfo.ServiceInitResult);
	}
	else
	{
		UE_LOG_EOSSDK_INFO("DesktopCrossplayStatusInfo (EOS_Platform_GetDesktopCrossplayStatus failed: %s)", *EOK_LexToString(Result));
	}
...
void FEOKSDKManager::LogAuthInfo(const EOS_HPlatform Platform, const EOS_EpicAccountId LoggedInAccount, int32 Indent) const
{
	const EOS_HAuth AuthHandle = EOS_Platform_GetAuthInterface(Platform);
	
	UE_LOG_EOSSDK_INFO("LoginStatus=%s", EOK_LexToString(EOS_Auth_GetLoginStatus(AuthHandle, LoggedInAccount)));
	...
	if (Result == EOS_EResult::EOS_Success)
	{
		...
		UE_LOG_EOSSDK_INFO("AuthType=%s", EOK_LexToString(AuthToken->AuthType));
#if !UE_BUILD_SHIPPING
	...
	{
		UE_LOG_EOSSDK_INFO("AuthToken (EOS_Auth_CopyUserAuthToken failed: %s)", *EOK_LexToString(Result));
	}
	...
	{
		UE_LOG_EOSSDK_INFO("IdToken (EOS_Auth_CopyIdToken failed: %s)", *EOK_LexToString(Result));
	}
#endif // !UE_BUILD_SHIPPING
}

void FEOKSDKManager::LogUserInfo(const EOS_HPlatform Platform, const EOS_EpicAccountId LoggedInAccount, const EOS_EpicAccountId TargetAccount, int32 Indent) const
{
	UE_LOG_EOSSDK_INFO("EpicAccountId=%s", *EOK_LexToString(TargetAccount));
	...
	{
		UE_LOG_EOSSDK_INFO("UserInfo (EOS_UserInfo_CopyUserInfo failed: %s)", *EOK_LexToString(Result));
	}
}

void FEOKSDKManager::LogPresenceInfo(const EOS_HPlatform Platform, const EOS_EpicAccountId LoggedInAccount, const EOS_EpicAccountId TargetAccount, int32 Indent) const
{
	...
	if (Result == EOS_EResult::EOS_Success)
	{
		...
		Indent++;
		UE_LOG_EOSSDK_INFO("Status=%s", EOK_LexToString(PresenceInfo->Status));
	...
	{
		UE_LOG_EOSSDK_INFO("Presence (EOS_Presence_CopyPresence failed: %s)", *EOK_LexToString(Result));
	}
}

void FEOKSDKManager::LogFriendsInfo(const EOS_HPlatform Platform, const EOS_EpicAccountId LoggedInAccount, int32 Indent) const
{
	...
	for (int32_t FriendIndex = 0; FriendIndex < FriendsCount; FriendIndex++)
	{
		...
		UE_LOG_EOSSDK_INFO("FriendStatus=%s", EOK_LexToString(FriendStatus));
...
void FEOKSDKManager::LogConnectInfo(const EOS_HPlatform Platform, const EOS_ProductUserId LoggedInAccount, int32 Indent) const
{
	UE_LOG_EOSSDK_INFO("ProductUserId=%s", *EOK_LexToString(LoggedInAccount));

	const EOS_HConnect ConnectHandle = EOS_Platform_GetConnectInterface(Platform);
	UE_LOG_EOSSDK_INFO("LoginStatus=%s", EOK_LexToString(EOS_Connect_GetLoginStatus(ConnectHandle, LoggedInAccount)));
	...
	{
		UE_LOG_EOSSDK_INFO("IdToken (EOS_Connect_CopyIdToken failed: %s)", *EOK_LexToString(Result));
	}
#endif // !UE_BUILD_SHIPPING
	...
	for (uint32_t ExternalAccountIndex = 0; ExternalAccountIndex < ExternalAccountCount; ExternalAccountIndex++)
	{
		...
		if (Result == EOS_EResult::EOS_Success)
		{
			...
			UE_LOG_EOSSDK_INFO("AccountIdType=%s", EOK_LexToString(ExternalAccountInfo->AccountIdType));
		...
		{
			UE_LOG_EOSSDK_INFO("ExternalAccountInfo (EOS_Connect_CopyProductUserExternalAccountByIndex failed: %s)", *EOK_LexToString(Result));
		}
...
void FEOKSDKManager::AddCallbackObject(TUniquePtr<FCallbackBase> CallbackObj)
{
	CallbackObjects.Emplace(MoveTemp(CallbackObj));
}

FEOKPlatformHandle::~FEOKPlatformHandle()
{
	Manager.ReleasePlatform(PlatformHandle);
}

void FEOKPlatformHandle::Tick()
{
	LLM_SCOPE(ELLMTag::RealTimeCommunications);
	QUICK_SCOPE_CYCLE_COUNTER(FEOKPlatformHandle_Tick);
	CSV_SCOPED_TIMING_STAT_EXCLUSIVE(EOSSDK);
	EOS_Platform_Tick(PlatformHandle);
}

FString FEOKPlatformHandle::GetOverrideCountryCode() const
{
	return Manager.GetOverrideCountryCode(PlatformHandle);
}

FString FEOKPlatformHandle::GetOverrideLocaleCode() const
{
	return Manager.GetOverrideLocaleCode(PlatformHandle);
}

void FEOKPlatformHandle::LogInfo(int32 Indent) const
{
	Manager.LogPlatformInfo(PlatformHandle, Indent);
}

void FEOKPlatformHandle::LogAuthInfo(const EOS_EpicAccountId LoggedInAccount, int32 Indent) const
{
	Manager.LogAuthInfo(PlatformHandle, LoggedInAccount, Indent);
}

void FEOKPlatformHandle::LogUserInfo(const EOS_EpicAccountId LoggedInAccount, const EOS_EpicAccountId TargetAccount, int32 Indent) const
{
	Manager.LogUserInfo(PlatformHandle, LoggedInAccount, TargetAccount, Indent);
}

void FEOKPlatformHandle::LogPresenceInfo(const EOS_EpicAccountId LoggedInAccount, const EOS_EpicAccountId TargetAccount, int32 Indent) const
{
	Manager.LogPresenceInfo(PlatformHandle, LoggedInAccount, TargetAccount, Indent);
}

void FEOKPlatformHandle::LogFriendsInfo(const EOS_EpicAccountId LoggedInAccount, int32 Indent) const
{
	Manager.LogFriendsInfo(PlatformHandle, LoggedInAccount, Indent);
}

void FEOKPlatformHandle::LogConnectInfo(const EOS_ProductUserId LoggedInAccount, int32 Indent) const
{
