...
#include "eos_sdk.h"
#include "EOKVoiceChatSynthComponent.h"
...
namespace
{
	...
	using FAudioBeforeSendCallback = TEOKGlobalCallback<EOS_RTCAudio_OnAudioBeforeSendCallback, EOS_RTCAudio_AudioBeforeSendCallbackInfo, FEOSVoiceChatUser>;
#else
	using FAudioBeforeSendCallback = TEOKGlobalCallback<EOS_RTCAudio_OnAudioBeforeSendCallback, EOS_RTCAudio_AudioBeforeSendCallbackInfo>;
#endif
}
...
bool FEOSVoiceChatUser::AddLobbyRoom(const FString& LobbyId)
{
	...
	if (EosResult == EOS_EResult::EOS_Success)
	{
		...
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("EOS_Lobby_IsLobbyRTCRoomConnected failed error=%s"), *EOK_LexToString(EosResult));
		}
	...
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("EOS_Lobby_GetLobbyRTCRoomName failed error=%s"), *EOK_LexToString(EosResult));
	}
...
void FEOSVoiceChatUser::RtcRegisterUser(const FString& UserId, const FOnVoiceChatUserRtcRegisterUserCompleteDelegate& Delegate)
{
	...
	if (EosResult != EOS_EResult::EOS_Success)
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("RtcRegisterUser failed: %s"), *EOK_LexToString(EosResult));
	}
...
void FEOSVoiceChatUser::RtcUnregisterUser(const FString& UserId, const FOnVoiceChatUserRtcUnregisterUserCompleteDelegate& Delegate)
{
	...
	if (EosResult != EOS_EResult::EOS_Success)
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("RtcUnregisterUser failed: %s"), *EOK_LexToString(EosResult));
	}
...
void FEOSVoiceChatUser::ApplyAudioInputOptions()
{
	if (IsLoggedIn())
	{
		...
		if (EosResult != EOS_EResult::EOS_Success)
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("ApplyAudioInputOptions failed: %s"), *EOK_LexToString(EosResult));
		}
	}
}
...
void FEOSVoiceChatUser::ApplyAudioOutputOptions()
{
	if (IsLoggedIn())
	{
		...
		if (EosResult != EOS_EResult::EOS_Success)
		{
			EOSVOICECHATUSER_LOG(Warning, TEXT("ApplyAudioOutputOptions failed: %s"), *EOK_LexToString(EosResult));
		}
	}
}
...
#if ENGINE_MAJOR_VERSION == 5
		// Protect against callbacks occurring after this object is destroyed, by wrapping in a TEOKGlobalCallback. This can occur when LeaveRoom during Logout fails.
...
void FEOSVoiceChatUser::OnBlockParticipant(const EOS_RTC_BlockParticipantCallbackInfo* CallbackInfo)
{
	...
	const FString PlayerName = EOK_LexToString(CallbackInfo->ParticipantId);
	...
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("OnBlockUser ChannelName=[%s] PlayerName=[%s] Block=[%s] failed error=[%s]"), *ChannelName, *PlayerName, *LexToString(CallbackInfo->bBlocked), *EOK_LexToString(CallbackInfo->ResultCode));
	}
}
...
void FEOSVoiceChatUser::OnUpdateParticipantVolume(const EOS_RTCAudio_UpdateParticipantVolumeCallbackInfo* CallbackInfo)
{
	...
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("OnUpdateParticipantVolume ChannelName=[%s] PlayerName=[%s] failed error=[%s]"), *ChannelName, *PlayerName, *EOK_LexToString(ResultCode));
	}
}
...
void FEOSVoiceChatUser::OnUpdateReceivingAudio(const EOS_RTCAudio_UpdateReceivingCallbackInfo* CallbackInfo)
{
	...
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("OnUpdateReceiving ChannelName=[%s] PlayerName=[%s] failed error=[%s]"), *ChannelName, *PlayerName, *EOK_LexToString(ResultCode));
	}
}
...
void FEOSVoiceChatUser::OnUpdateSendingAudio(const EOS_RTCAudio_UpdateSendingCallbackInfo* CallbackInfo)
{
	...
	{
		EOSVOICECHATUSER_LOG(Warning, TEXT("OnUpdateSending ChannelName=[%s] failed error=[%s]"), *ChannelName, *EOK_LexToString(CallbackInfo->ResultCode));
	}
}
...
void FEOSVoiceChatUser::OnChannelDisconnected(const EOS_RTC_DisconnectedCallbackInfo* CallbackInfo)
{
	...
	if (FChannelSession* ChannelSession = LoginSession.ChannelSessions.Find(ChannelName))
	{
		...
		else
		{
			...
			TArray<UEOK_Voice_Subsystem*> Objects;
			for (TObjectIterator<UEOK_Voice_Subsystem> Itr; Itr; ++Itr)
			{
				Objects.Add(*Itr);
			}
			if(Objects[0])
...
void FEOSVoiceChatUser::OnLobbyChannelConnectionChanged(const EOS_Lobby_RTCRoomConnectionChangedCallbackInfo* CallbackInfo)
{
	...
	if (LocalUserId != LoginSession.LocalUserProductUserId)
	{
		EOSVOICECHATUSER_LOG(VeryVerbose, TEXT("OnLobbyChannelConnectionChanged LocalUserId=[%s] LobbyId=[%s] bIsConnected=[%s] Reason=[%s] Skipping notification for other user"),
			*EOK_LexToString(LocalUserId), *LobbyId, *LexToString(bIsConnected), *LexToString(Reason));
	}
	...
	if (CallbackInfo->ParticipantStatus == EOS_ERTCParticipantStatus::EOS_RTCPS_Joined)
	{
		if (FChannelSession* ChannelSession = LoginSession.ChannelSessions.Find(ChannelName))
		{
			if (PlayerName != ChannelSession->PlayerName)
			{
				...
				FEOKVoiceChatDelegates::OnVoiceChatPlayerAddedMetadata.Broadcast(LoginSession.PlayerName, ChannelName, PlayerName, Metadata);
	...
	if (FChannelSession* ChannelSession = LoginSession.ChannelSessions.Find(ChannelName))
	{
		if (FChannelParticipant* ChannelParticipant = ChannelSession->Participants.Find(PlayerName))
		{
			...
			if (!ChannelParticipant->AudioStatus.IsSet() || ChannelParticipant->AudioStatus.GetValue() != AudioStatus)
			{
				...
				FEOKVoiceChatDelegates::OnAudioStatusChanged.Broadcast(LoginSession.PlayerName, ChannelSession->ChannelName, PlayerName, AudioStatus);
			}
		}
		else
		{
		...
		if (Buffer->Frames)
		{
			...
			if(IsValid(GEngine))
			{
				if (const UWorld* World = GEngine->GetWorldContexts().Last().World())
				{
					...
					if(PlayerArray.Num() > 0)
					{
						for(int i=0; i<PlayerArray.Num(); i++)
						{
							if(APlayerState* PlayerState = PlayerArray[i])
							{
								...
								{
									const TSharedPtr<const FUniqueNetId> EOK_NetID = PlayerState->GetUniqueId().GetUniqueNetId();
									if(EOK_NetID.IsValid())
									{
										FString ProductId = EOK_NetID->ToString();
					...
					if(SpeakerActor)
					{
						TArray<UEOKVoiceChatSynthComponent*> VoiceChatSynthComponents;
						SpeakerActor->GetComponents<UEOKVoiceChatSynthComponent>(VoiceChatSynthComponents);
...
{
	EOSVOICECHATUSER_LOG(Log, TEXT("OnChannelAudioInputState ChannelName=[%s] State=[%s]"), UTF8_TO_TCHAR(CallbackInfo->RoomName), EOK_LexToString(CallbackInfo->Status));

	FEOKVoiceChatDelegates::OnAudioInputDeviceStatusChanged.Broadcast(LoginSession.PlayerName, CallbackInfo->Status);
}
