...
FVoiceChatResult ResultFromEOSResult(const EOS_EResult EosResult)
{
	...
	if (EosResult != EOS_EResult::EOS_Success)
	{
		...
		default:
			...
			Result = EOSVOICECHAT_ERROR(EVoiceChatResult::ImplementationError, *EOK_LexToString(EosResult));
			break;
		}

		Result.ErrorNum = static_cast<int>(EosResult);
		Result.ErrorDesc = FString::Printf(TEXT("EOS_EResult=%s"), *EOK_LexToString(EosResult));
	}
