...
const TCHAR* EOK_LexToString(EOS_ERTCAudioInputStatus Status)
{
	switch (Status)
	{
...
FEOKVoiceChatDelegates::FOnAudioInputDeviceStatusChanged FEOKVoiceChatDelegates::OnAudioInputDeviceStatusChanged;
FEOKVoiceChatDelegates::FOnVoiceChatChannelConnectionStateDelegate FEOKVoiceChatDelegates::OnVoiceChatChannelConnectionStateChanged;
FEOKVoiceChatDelegates::FOnVoiceChatPlayerAddedMetadataDelegate FEOKVoiceChatDelegates::OnVoiceChatPlayerAddedMetadata;
FEOKVoiceChatDelegates::FOnAudioStatusChanged FEOKVoiceChatDelegates::OnAudioStatusChanged;

int64 FEOSVoiceChat::StaticInstanceIdCount = 0;

#define EOS_VOICE_TODO 0

FEOSVoiceChat::FEOSVoiceChat(IEOSSDKManager& InSDKManager, const IEOKPlatformHandlePtr& InPlatformHandle)
		...
		if (bEnabled)
		{
			...
			if (!EosPlatformHandle)
			{
				...
				if (EosResult == EOS_EResult::EOS_Success)
				{
					...
					EOKSettings->GetSettingsForArtifact(EOKSettings->VoiceArtifactName, VoiceArtifactSettings);
...
IEOKPlatformHandlePtr FEOSVoiceChat::EOSPlatformCreate(EOS_Platform_Options& PlatformOptions)
{
