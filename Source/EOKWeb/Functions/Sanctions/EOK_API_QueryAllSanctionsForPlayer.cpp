...
#include "EOK_API_QueryAllSanctionsForPlayer.h"

UEOK_API_QueryAllSanctionsForPlayer* UEOK_API_QueryAllSanctionsForPlayer::QueryAllSanctionsForPlayer(
	FString Authorization, FString DeploymentId, FString ProductUserId, int32 Limit, int32 Offset)
{
	UEOK_API_QueryAllSanctionsForPlayer* Proxy = NewObject<UEOK_API_QueryAllSanctionsForPlayer>();
...
void UEOK_API_QueryAllSanctionsForPlayer::Activate()
{
	Super::Activate();
	...
	HttpRequest->OnProcessRequestComplete().BindUObject(this, &UEOK_API_QueryAllSanctionsForPlayer::OnResponseReceived);
