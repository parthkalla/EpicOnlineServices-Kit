...
#include "EOK_API_ApprovePendingSanctions.h"

UEOK_API_ApprovePendingSanctions* UEOK_API_ApprovePendingSanctions::ApprovePendingSanctions(FString Authorization,
	FString DeploymentId, FString EOS_OrganizationId, FString EOS_ClientId, FString EOS_DeploymentId,
	TArray<FString> ReferenceIds)
{
	UEOK_API_ApprovePendingSanctions* Node = NewObject<UEOK_API_ApprovePendingSanctions>();
...
void UEOK_API_ApprovePendingSanctions::Activate()
{
	Super::Activate();
	...
	HttpRequest->OnProcessRequestComplete().BindUObject(this, &UEOK_API_ApprovePendingSanctions::OnResponseReceived);
