...
#include "EOK_API_QueryProductUsers.h"

UEOK_API_QueryProductUsers* UEOK_API_QueryProductUsers::QueryProductUsers(FString Authorization,
	TArray<FString> ProductUserIds)
{
	UEOK_API_QueryProductUsers* Node = NewObject<UEOK_API_QueryProductUsers>();
	Node->Var_Authorization = Authorization;
	Node->Var_ProductUserIds = ProductUserIds;
	return Node;
}

void UEOK_API_QueryProductUsers::Activate()
{
	Super::Activate();
	...
	URL += QueryString;
	HttpRequest->OnProcessRequestComplete().BindUObject(this, &UEOK_API_QueryProductUsers::OnResponseReceived);
