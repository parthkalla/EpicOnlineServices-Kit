...
#include "EOK_API_QueryExternalAccounts.h"

UEOK_API_QueryExternalAccounts* UEOK_API_QueryExternalAccounts::QueryExternalAccounts(FString Authorization,
	TArray<FString> AccountId, FString IdentityProviderId, FString Environment)
{
	UEOK_API_QueryExternalAccounts* Proxy = NewObject<UEOK_API_QueryExternalAccounts>();
...
void UEOK_API_QueryExternalAccounts::Activate()
{
	Super::Activate();
	...
	URL += QueryString;
	HttpRequest->OnProcessRequestComplete().BindUObject(this, &UEOK_API_QueryExternalAccounts::OnResponseReceived);
