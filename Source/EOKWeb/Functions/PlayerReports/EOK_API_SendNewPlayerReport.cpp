...
#include "EOK_API_SendNewPlayerReport.h"

UEOK_API_SendNewPlayerReport* UEOK_API_SendNewPlayerReport::SendNewPlayerReport(FString Authorization, FString ReportingPlayerId, FString ReportedPlayerId, FString Time, int32 ReasonId, FString Message, FString Context)
{
	UEOK_API_SendNewPlayerReport* Node = NewObject<UEOK_API_SendNewPlayerReport>();
...
void UEOK_API_SendNewPlayerReport::Activate()
{
	Super::Activate();
	...
	HttpRequest->OnProcessRequestComplete().BindUObject(this, &UEOK_API_SendNewPlayerReport::OnResponseReceived);
