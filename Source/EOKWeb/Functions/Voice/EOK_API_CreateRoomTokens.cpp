...
#include "EOK_API_CreateRoomTokens.h"


UEOK_API_CreateRoomTokens* UEOK_API_CreateRoomTokens::CreateVoiceRoomToken(FString Authorization, FString DeploymentId,
                                                                           FString RoomId, TArray<FEWebApi_EosRoomParticipant> Participants)
{
	UEOK_API_CreateRoomTokens* Proxy = NewObject<UEOK_API_CreateRoomTokens>();
...
void UEOK_API_CreateRoomTokens::Activate()
{
	...
	HttpRequest->OnProcessRequestComplete().BindUObject(this, &UEOK_API_CreateRoomTokens::OnResponseReceived);
