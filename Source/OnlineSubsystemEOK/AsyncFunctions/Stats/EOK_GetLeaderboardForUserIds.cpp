...
#include "EOK_GetLeaderboardForUserIds.h"
...
UEOK_GetLeaderboardForUserIds* UEOK_GetLeaderboardForUserIds::GetEOKLeaderboardForUserIds(TArray<FString> TargetProductUserIds, FString LocalProductUserId, bool UseTime, FDateTime StartTime, FDateTime EndTime, ELeaderboardAggregation AggregationType, FString StatName)
{
	UEOK_GetLeaderboardForUserIds* GetLeaderboardForUserIdsObj = NewObject < UEOK_GetLeaderboardForUserIds >();
...
void UEOK_GetLeaderboardForUserIds::GetLeaderboardForUserIds()
{
    if (IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get())
    {
        if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
        {
            if (EOSRef->LeaderboardsHandle != nullptr)
            {
                ...
                EOS_Leaderboards_QueryLeaderboardUserScores(EOSRef->LeaderboardsHandle, &QueryOptions, this, &UEOK_GetLeaderboardForUserIds::GetLeaderboardForUserIdsCallback);
...
void UEOK_GetLeaderboardForUserIds::GetLeaderboardForUserIdsCallback(const EOS_Leaderboards_OnQueryLeaderboardUserScoresCompleteCallbackInfo* Data)
{
    if (Data->ClientData)
    {
        // Check if the client data is valid
        if (UEOK_GetLeaderboardForUserIds* GetLeaderboardForUserIdsFunc = static_cast<UEOK_GetLeaderboardForUserIds*>(Data->ClientData))
        {
...
void UEOK_GetLeaderboardForUserIds::ResultSuccess()
{
    if (IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get())
    {
        if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
        {
            if (EOSRef->LeaderboardsHandle != nullptr)
            {
                TArray<FEOKExtendedLeaderboardValue> EOKExtendedLeaderboardValues;
                ...
                for (uint32_t Index = 0; Index < LeaderboardRecordCount; ++Index)
                {
                    ...
                    {
                        FEOKExtendedLeaderboardValue TempEOKExtendedLeaderboardValue;

                        TempEOKExtendedLeaderboardValue.Score = OutLeaderboardRecord->Score;
                        ...
                        {
                            TempEOKExtendedLeaderboardValue.UserId = (UTF8_TO_TCHAR(PUIDString));
                        }    

                        EOKExtendedLeaderboardValues.Add(TempEOKExtendedLeaderboardValue);
                ...
                Success.Broadcast(EOKExtendedLeaderboardValues);
...
void UEOK_GetLeaderboardForUserIds::ResultFaliure()
{
    Faliure.Broadcast(TArray<FEOKExtendedLeaderboardValue>());
    SetReadyToDestroy();
#if ENGINE_MAJOR_VERSION == 5
    MarkAsGarbage();
#else
...
void UEOK_GetLeaderboardForUserIds::Activate()
{
