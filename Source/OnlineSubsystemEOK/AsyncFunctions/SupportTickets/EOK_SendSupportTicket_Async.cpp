...
void UEOK_SendSupportTicket_Async::Activate()
{
	SendTicketRequest();
	Super::Activate();
}

UEOK_SendSupportTicket_Async* UEOK_SendSupportTicket_Async::SendEOKTicketRequestAsyncFunction(FSupportTicketData TicketData)
{
	UEOK_SendSupportTicket_Async* UEOK_SendEOKTicketRequest = NewObject < UEOK_SendSupportTicket_Async>();

	UEOK_SendEOKTicketRequest->Var_TicketData = TicketData;

	return UEOK_SendEOKTicketRequest;
}

void UEOK_SendSupportTicket_Async::SendTicketRequest()
{
    ...
    {
        Failure.Broadcast(TEXT("Couldn't retrive your api key from EOK settings."), FSupportTicketResponseData(), 1);
        SetReadyToDestroy();
#if ENGINE_MAJOR_VERSION == 5
        MarkAsGarbage();
#else
    ...
    Request->OnProcessRequestComplete().BindUObject(this, &UEOK_SendSupportTicket_Async::OnResponseReceived);
    Request->ProcessRequest();
}

void UEOK_SendSupportTicket_Async::OnResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
