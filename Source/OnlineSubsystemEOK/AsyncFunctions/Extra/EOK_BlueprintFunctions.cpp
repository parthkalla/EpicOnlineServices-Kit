...
#include "EOK_BlueprintFunctions.h"
...
#include "EOKVoiceChat/Private/Android/AndroidEOSVoiceChatUser.h"
#include "GameFramework/GameModeBase.h"
#include "Interfaces/IHttpRequest.h"
#include "Interfaces/IHttpResponse.h"
#include "Interfaces/IPluginManager.h"
#include "Misc/Base64.h"
#include "OnlineSubsystemEOK/AsyncFunctions/Beacons/PingHost.h"
#include "OnlineSubsystemEOK/AsyncFunctions/Beacons/PingHostObject.h"
#include "OnlineSubsystemEOK/AsyncFunctions/Login/EOK_Login_AsyncFunction.h"

FString UEOK_BlueprintFunctions::GetEpicAccountId(UObject* Context)
{
	if(Context)
	{
		if(!Context->GetWorld())
		...
		if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
		{
			...
			{
				if(const TSharedPtr<const FUniqueNetId> EOK_NetID = UGameplayStatics::GetGameInstance(Context->GetWorld())->GetFirstGamePlayer()->GetPreferredUniqueNetId().GetUniqueNetId())
				{
					const FString String_UserID = EOK_NetID.Get()->ToString();
...
FEOK_CurrentSessionInfo UEOK_BlueprintFunctions::GetCurrentSessionInfo(UObject* Context, bool& bIsSessionPresent,FName SessionName)
{
	...
	if(Context)
	{
		if(!Context->GetWorld())
		{
			return FEOK_CurrentSessionInfo();
		}
		if(const IOnlineSubsystem *SubsystemRef = Online::GetSubsystem(Context->GetWorld(), "EOK"))
		{
			if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
			{
				if(!SessionPtrRef->GetNamedSession(SessionName))
				{
					UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetCurrentSessionInfo: Session not found"));
					return FEOK_CurrentSessionInfo();
				}
				bIsSessionPresent = true;
				FEOK_CurrentSessionInfo SessionInfo(*SessionPtrRef->GetNamedSession(SessionName));
				return SessionInfo;
			}
			return FEOK_CurrentSessionInfo();
		}
...
TArray<FName> UEOK_BlueprintFunctions::GetAllCurrentSessionNames(UObject* Context)
{
	if(Context)
	{
		if(!Context->GetWorld())
...
FString UEOK_BlueprintFunctions::GetProductUserID(UObject* Context)
{
	if(Context)
	{
		if(!Context->GetWorld())
...
IVoiceChatUser* UEOK_BlueprintFunctions::GetLobbyVoiceChat(UObject* Context)
{
	if (Context)
	{
		...
		if (!World)
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetLobbyVoiceChat: World is null"));
			return nullptr;
		}

		IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK");
		if (!OnlineSub)
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetLobbyVoiceChat: OnlineSubsystem is null"));
			return nullptr;
		}
		...
		if (!EOSRef)
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetLobbyVoiceChat: EOSRef is null"));
			return nullptr;
		}
		...
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetLobbyVoiceChat: IdentityPointerRef is null"));
			return nullptr;
		}
		...
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetLobbyVoiceChat: UniquePlayerId is null"));
			return nullptr;
		}
...
bool UEOK_BlueprintFunctions::MuteLobbyVoiceChat(UObject* Context, bool bMute)
{
	if(GetLobbyVoiceChat(Context))
	{
		...
		if (!OnlineSub)
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::MuteLobbyVoiceChat: OnlineSubsystem is null"));
			return false;
		}
		IOnlineIdentityPtr IdentityPointerRef = OnlineSub->GetIdentityInterface();
		if (!IdentityPointerRef.IsValid())
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::MuteLobbyVoiceChat: IdentityPointerRef is null"));
			return false;
		}
		GetLobbyVoiceChat(Context)->SetPlayerMuted(*IdentityPointerRef->GetUniquePlayerId(0)->ToString(), bMute);
		return true;
	}
	UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::MuteLobbyVoiceChat: VoiceChatUserInterface is null"));
	return false;
}


bool UEOK_BlueprintFunctions::IsLobbyVoiceChatMuted(UObject* Context)
{
	if(GetLobbyVoiceChat(Context))
	{
		IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK");
		if (!OnlineSub)
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::IsLobbyVoiceChatMuted: OnlineSubsystem is null"));
			return false;
		}
		IOnlineIdentityPtr IdentityPointerRef = OnlineSub->GetIdentityInterface();
		if (!IdentityPointerRef.IsValid())
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::IsLobbyVoiceChatMuted: IdentityPointerRef is null"));
			return false;
		}
		UE_LOG(LogEOK, Verbose, TEXT("UEOK_BlueprintFunctions::IsLobbyVoiceChatMuted: Checking if player is muted"));
		return GetLobbyVoiceChat(Context)->IsPlayerMuted(*IdentityPointerRef->GetUniquePlayerId(0)->ToString());
	}
	UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::IsLobbyVoiceChatMuted: Context is null"));
	return false;
}

bool UEOK_BlueprintFunctions::SetLobbyOutputMethod(UObject* Context, FString MethodID)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::SetLobbyInputMethod(UObject* Context, FString MethodID)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::BlockLobbyVoiceChatPlayers(UObject* Context, TArray<FString> BlockedPlayers)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::UnblockLobbyVoiceChatPlayers(UObject* Context, TArray<FString> UnblockedPlayers)
{
	if(GetLobbyVoiceChat(Context))
	{
...
float UEOK_BlueprintFunctions::GetLobbyVoiceChatOutputVolume(UObject* Context)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::SetLobbyVoiceChatOutputVolume(UObject* Context, float Volume)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::SetLobbyVoiceChatInputVolume(UObject* Context, float Volume)
{
	if(GetLobbyVoiceChat(Context))
	{
		IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK");
...
float UEOK_BlueprintFunctions::GetLobbyVoiceChatInputVolume(UObject* Context)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::SetLobbyPlayerVoiceChatVolume(UObject* Context, FString PlayerName, float Volume)
{
	if(GetLobbyVoiceChat(Context))
	{
		GetLobbyVoiceChat(Context)->SetPlayerVolume(PlayerName, Volume);
		UE_LOG(LogEOK, Verbose, TEXT("UEOK_BlueprintFunctions::SetLobbyPlayerVoiceChatVolume: Player volume set"));
		return true;
	}
	UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::SetLobbyPlayerVoiceChatVolume: VoiceChatUserInterface is null"));
	return false;
}

float UEOK_BlueprintFunctions::GetLobbyPlayerVoiceChatVolume(UObject* Context, FString PlayerName)
{
	if(GetLobbyVoiceChat(Context))
	{
...
bool UEOK_BlueprintFunctions::ShowFriendsList()
{
	const IOnlineSubsystem* OnlineSubsystem = IOnlineSubsystem::Get("EOK"); // Get the Online Subsystem
...
FEOKUniqueNetId UEOK_BlueprintFunctions::MakeEOKUniqueNetId(FString EpicAccountId, FString ProductUserId)
{
	if(ProductUserId.IsEmpty())
	{
		return FEOKUniqueNetId();
	}
	if (IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
...
bool UEOK_BlueprintFunctions::AcceptSessionInvite(FString InviteId, FString LocalUserId, FString InviterUserId)
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
...
bool UEOK_BlueprintFunctions::RejectSessionInvite(FString InviteId, FString LocalUserId)
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
...
bool UEOK_BlueprintFunctions::StartSession(FName SessionName)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
bool UEOK_BlueprintFunctions::RegisterPlayer(FName SessionName,FEOKUniqueNetId PlayerId, bool bWasInvited)
{
	if(!PlayerId.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::RegisterPlayer: PlayerId is not valid"));
		return false;
	}
	if(!PlayerId.UniqueNetId.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::RegisterPlayer: PlayerId.UniqueNetId is not valid"));
		return false;
	}
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
...
bool UEOK_BlueprintFunctions::UnRegisterPlayer(FName SessionName, FEOKUniqueNetId PlayerId)
{
	if(!PlayerId.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::UnRegisterPlayer: PlayerId is not valid"));
		return false;
	}
	if(!PlayerId.UniqueNetId.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::UnRegisterPlayer: PlayerId.UniqueNetId is not valid"));
		return false;
	}
...
bool UEOK_BlueprintFunctions::EndSession(FName SessionName)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
bool UEOK_BlueprintFunctions::IsInSession(FName SessionName,FEOKUniqueNetId PlayerId)
{
	if(!PlayerId.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::IsInSession: PlayerId is not valid"));
		return false;
	}
	if(!PlayerId.UniqueNetId.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::IsInSession: PlayerId.UniqueNetId is not valid"));
		return false;
	}
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
...
FString UEOK_BlueprintFunctions::GetPlayerNickname(const int32 LocalUserNum)
{
	FString Nickname;
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
...
EEOK_LoginStatus UEOK_BlueprintFunctions::GetLoginStatus(const int32 LocalUserNum)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			...
			case ELoginStatus::UsingLocalProfile:
				return EEOK_LoginStatus::UsingLocalProfile;
				break;
			case ELoginStatus::LoggedIn:
				return EEOK_LoginStatus::LoggedIn;
				break;
			default:
				return EEOK_LoginStatus::NotLoggedIn;
			}
...
FString UEOK_BlueprintFunctions::GenerateSessionCode(int32 CodeLength)
{
	FString SessionCode;
...
bool UEOK_BlueprintFunctions::IsEOKActive()
{
	if (const IOnlineSubsystem* OnlineSubsystem = IOnlineSubsystem::Get("EOK"))
	{
		return true;
	}
	return false;
...
FName UEOK_BlueprintFunctions::GetActiveSubsystem()
{
...
FString UEOK_BlueprintFunctions::ByteArrayToString(const TArray<uint8>& DataToConvert)
{
...
TArray<uint8> UEOK_BlueprintFunctions::StringToByteArray(const FString& DataToConvert)
{
...
FEOKUniqueNetId UEOK_BlueprintFunctions::GetUserUniqueID(const APlayerController* PlayerController, bool& bIsValid)
{
	...
	{
		if(const TSharedPtr<const FUniqueNetId> EOK_NetID = PlayerState->GetUniqueId().GetUniqueNetId())
		{
			FEOKUniqueNetId LocalUNetID;
			LocalUNetID.SetUniqueNetId(EOK_NetID);
	...
	if(const TSharedPtr<const FUniqueNetId> EOK_NetID = IdentityInterface->GetUniquePlayerId(0))
	{
		FEOKUniqueNetId LocalUNetID;
		LocalUNetID.SetUniqueNetId(EOK_NetID);
		if(LocalUNetID.IsValid())
...
FEOKUniqueNetId UEOK_BlueprintFunctions::GetUserUniqueIDFromPlayerState(const APlayerState* PlayerState, bool& bIsValid)
{
	if(!PlayerState)
	{
		bIsValid = false;
...
FString UEOK_BlueprintFunctions::GetEOSSDKVersion()
{
	return EOS_GetVersion();
}

FString UEOK_BlueprintFunctions::GetEOKPluginVersion()
{
	const TSharedPtr<IPlugin> Plugin = IPluginManager::Get().FindPlugin(TEXT("EpicOnlineServices-Kit V2"));
	if(Plugin.IsValid())
	{
...
bool UEOK_BlueprintFunctions::IsValidSession(FSessionFindStruct Session)
{
...
FString UEOK_BlueprintFunctions::GetCurrentPort(AGameModeBase* CurrentGameMode)
{
	if(!CurrentGameMode)
	{
		return FString();
...
void UEOK_BlueprintFunctions::MakeSanctionAppeal(FString AccessToken, EEOSSanctionType Reason,const FOnResponseFromSanctions& OnResponseFromSanctions)
{
	FEOSArtifactSettings ArtifactSettings;
	UEOKSettings::GetSettingsForArtifact(UEOKSettings::GetSettings().DefaultArtifactName, ArtifactSettings);
...
void UEOK_BlueprintFunctions::RequestEOSAccessToken(const FOnResponseFromEpicForAccessToken& Response)
{
...
FDateTime UEOK_BlueprintFunctions::ConvertPosixTimeToDateTime(int64 PosixTime)
{
	return FDateTime::FromUnixTimestamp(PosixTime);
}

FString UEOK_BlueprintFunctions::GetResolvedConnectString(FName SessionName)
{
	if (const IOnlineSubsystem* SubsystemRef = IOnlineSubsystem::Get("EOK"))
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
bool UEOK_BlueprintFunctions::GetAutoLoginAttemptedStatus()
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
	...
	UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetAutoLoginAttemptedStatus: OnlineSubsystem is null"));
	return false;
}

bool UEOK_BlueprintFunctions::GetAutoLoginInProgressStatus()
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
	...
	UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::GetAutoLoginInProgressStatus: OnlineSubsystem is null"));
	return false;
}

FString UEOK_BlueprintFunctions::GetEnvironmentVariable(const FString& EnvVariableName)
{
...
bool UEOK_BlueprintFunctions::InitPingBeacon(UObject* Context, AGameModeBase* GameMode)
{
	if(Context)
	{
		if(Context->GetWorld() && GameMode)
		{
			if(GameMode->GetNetMode() == NM_DedicatedServer || GameMode ->GetNetMode() == NM_ListenServer)
			{
				...
				if(Host && Host->InitializeHost())
				{
					...
					{
						UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::InitPingBeacon: HostObject is null or failed to initialize"));
					}
				}
				else
				{
					UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::InitPingBeacon: Host is null or failed to initialize"));
				}
			}
			else
			{
				UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::InitPingBeacon: GameMode is not a server"));
			}
		}
		else
		{
			UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::InitPingBeacon: Context or GameMode is null"));
		}
	}
	else
	{
		UE_LOG(LogEOK, Error, TEXT("UEOK_BlueprintFunctions::InitPingBeacon: Context is null"));
	}
	return false;
}
