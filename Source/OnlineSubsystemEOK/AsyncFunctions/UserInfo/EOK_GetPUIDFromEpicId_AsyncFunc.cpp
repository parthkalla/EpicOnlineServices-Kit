...
#include "EOK_GetPUIDFromEpicId_AsyncFunc.h"
...
UEOK_GetPUIDFromEpicId_AsyncFunc* UEOK_GetPUIDFromEpicId_AsyncFunc::EOKGetPUIDFromEpicId(TArray<FString> TargetEpicAccountIds, FString LocalProductUserId)
{
	UEOK_GetPUIDFromEpicId_AsyncFunc* EOK_GetPUIDFromEpicId_Obj = NewObject<UEOK_GetPUIDFromEpicId_AsyncFunc>();

	EOK_GetPUIDFromEpicId_Obj->Var_TargetUserIds = TargetEpicAccountIds;
	EOK_GetPUIDFromEpicId_Obj->Var_LocalProductUserId = LocalProductUserId;

	return EOK_GetPUIDFromEpicId_Obj;
}

void UEOK_GetPUIDFromEpicId_AsyncFunc::GetPUIDFromEpicId()
{
...
void UEOK_GetPUIDFromEpicId_AsyncFunc::OnQueryExternalAccountMappingsComplete(const EOS_Connect_QueryExternalAccountMappingsCallbackInfo* Data)
{
	if (Data->ResultCode == EOS_EResult::EOS_Success)
	{
		if (UEOK_GetPUIDFromEpicId_AsyncFunc* GetPUIDFromEpicId = static_cast<UEOK_GetPUIDFromEpicId_AsyncFunc*>(Data->ClientData))
		{
...
void UEOK_GetPUIDFromEpicId_AsyncFunc::QueryExternalAccountMappingsSuccess()
{
	if (IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get())
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
...
void UEOK_GetPUIDFromEpicId_AsyncFunc::QueryExternalAccountMappingsFailure()
{
	TArray<FProductUserIdAndEpicId> EmptyProductUserIdAndEpicId;
...
void UEOK_GetPUIDFromEpicId_AsyncFunc::Activate()
{
	GetPUIDFromEpicId();
	Super::Activate();
}
