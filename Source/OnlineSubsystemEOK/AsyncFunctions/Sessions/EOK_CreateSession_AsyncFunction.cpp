...
#include "EOK_CreateSession_AsyncFunction.h"
...
void UEOK_CreateSession_AsyncFunction::Activate()
{
	CreateSession();
	Super::Activate();
}

void UEOK_CreateSession_AsyncFunction::CreateSession()
{
	if(IOnlineSubsystem *SubsystemRef = Online::GetSubsystem(this->GetWorld(), "EOK"))
	{
		if(IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
		{
			...
			SessionPtrRef->OnCreateSessionCompleteDelegates.AddUObject(this, &UEOK_CreateSession_AsyncFunction::OnCreateSessionCompleted);
...
void UEOK_CreateSession_AsyncFunction::OnCreateSessionCompleted(FName SessionName, bool bWasSuccessful)
{
	if(bWasSuccessful)
	{
		if(bDelegateCalled == false)
...
UEOK_CreateSession_AsyncFunction* UEOK_CreateSession_AsyncFunction::CreateEOKSession(
...
{
	UEOK_CreateSession_AsyncFunction* UEOK_CreateSessionObject= NewObject<UEOK_CreateSession_AsyncFunction>();
	UEOK_CreateSessionObject->SessionSettings = SessionSettings;
	UEOK_CreateSessionObject->NumberOfPublicConnections = NumberOfPublicConnections;
	UEOK_CreateSessionObject->DedicatedServerSettings = DedicatedServerSettings;
	UEOK_CreateSessionObject->VSessionName = SessionName;
	UEOK_CreateSessionObject->ExtraSettings = ExtraSettings;
	return UEOK_CreateSessionObject;
}
