...
void FOnlineSubsystemEOS::ModuleInit()
{
	...
	const FName EOSSharedModuleName = TEXT("EOKShared");
...
bool FOnlineSubsystemEOS::PlatformCreate()
{
	...
	if (!UEOKSettings::GetSettingsForArtifact(ArtifactName, ArtifactSettings))
	{
...
bool FOnlineSubsystemEOS::Init()
{
	...
	bIsDefaultOSS = DefaultOSS == TEXT("EOK");
	bIsPlatformOSS = PlatformOSS == TEXT("EOK");
	...
	{
		UE_LOG(LogEOK, Log, TEXT("FOnlineSubsystemEOS: failed to init EOS platform, couldn't get anti cheat client handle. This is expected if you are not using Easy Anti Cheat."));
	}
	...
	{
		UE_LOG(LogEOK, Log, TEXT("FOnlineSubsystemEOS: failed to init EOS platform, couldn't get anti cheat server handle. This is expected if you are not using Easy Anti Cheat."));
	}
	...
	SocketSubsystem = MakeShareable(new FSocketSubsystemEOK(EOSPlatformHandle, MakeShareable(new FSocketSubsystemEOSUtils_OnlineSubsystemEOS(*this))));
...
void FOnlineSubsystemEOS::ReloadConfigs(const TSet<FString>& ConfigSections)
{
	...
	const bool bConfigChanged = ConfigSections.Find(GetDefault<UEOKSettings>()->GetClass()->GetPathName()) != nullptr;
	...
	if (bConfigChanged)
	{
		GetMutableDefault<UEOKSettings>()->ReloadConfig();
	}
...
{
	return NSLOCTEXT("OnlineSubsystemEOK", "OnlineServiceName", "EOK");
}

FOnlineSubsystemEOS::FOnlineSubsystemEOS(FName InInstanceName) :
	IOnlineSubsystemEOS("EOK", InInstanceName)
...
IVoiceChatUser* FOnlineSubsystemEOS::GetVoiceChatUserInterface(const FUniqueNetId& LocalUserId)
{
	...
	if(VoiceChatInterface && UserManager->IsLocalUser(LocalUserId))
	{
		...
		else
		{
			...
			VoiceChatUser->Login(UserManager->GetPlatformUserIdFromUniqueNetId(LocalUserId), EOK_LexToString(FUniqueNetIdEOS::Cast(LocalUserId).GetProductUserId()), FString(), FOnVoiceChatLoginCompleteDelegate());
