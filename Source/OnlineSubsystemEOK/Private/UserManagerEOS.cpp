...
typedef TEOKGlobalCallback<EOS_UI_OnDisplaySettingsUpdatedCallback, EOS_UI_OnDisplaySettingsUpdatedCallbackInfo, FUserManagerEOS> FOnDisplaySettingsUpdatedCallback;
typedef TEOKGlobalCallback<EOS_Connect_OnLoginStatusChangedCallback, EOS_Connect_LoginStatusChangedCallbackInfo, FUserManagerEOS> FConnectLoginStatusChangedCallback;
typedef TEOKGlobalCallback<EOS_Connect_OnAuthExpirationCallback, EOS_Connect_AuthExpirationCallbackInfo, FUserManagerEOS> FRefreshAuthCallback;
#else
typedef TEOKGlobalCallback<EOS_UI_OnDisplaySettingsUpdatedCallback, EOS_UI_OnDisplaySettingsUpdatedCallbackInfo> FOnDisplaySettingsUpdatedCallback;
typedef TEOKGlobalCallback<EOS_Connect_OnLoginStatusChangedCallback, EOS_Connect_LoginStatusChangedCallbackInfo> FConnectLoginStatusChangedCallback;
typedef TEOKGlobalCallback<EOS_Connect_OnAuthExpirationCallback, EOS_Connect_AuthExpirationCallbackInfo> FRefreshAuthCallback;
#endif
...
void FUserManagerEOS::GetPlatformAuthToken(int32 LocalUserNum, const FOnGetLinkedAccountAuthTokenCompleteDelegate& Delegate) const
{
	...
	// CRITICAL: Unreal Engine 5.4's Steam OnlineIdentityInterface should use GetAuthTicketForWebApi
	// with "epiconlineservices" when TokenType="Session" is passed to GetLinkedAccountAuthToken.
	// 
	// If you're getting "Invalid session ticket" errors (EOS_Connect_ExternalTokenValidationFailed),
	// this likely means Unreal Engine's Steam subsystem is NOT using "epiconlineservices" as the
	// identity string when calling GetAuthTicketForWebApi.
	//
	// VERIFICATION CHECKLIST:
	// 1. EOS Dev Portal: Steam Identity Provider -> SteamNetworkingIdentity = "epiconlineservices" ✓
	// 2. EOS Dev Portal: Steam Identity Provider -> App ID = 4097280 ✓
	// 3. DefaultEngine.ini: SteamAppId = 4097280 ✓
	// 4. Unreal Engine 5.4 Steam subsystem must use GetAuthTicketForWebApi("epiconlineservices")
	//
	// SOLUTION: If Unreal Engine 5.4's Steam subsystem doesn't use "epiconlineservices",
	// you may need to:
	// - Update to a newer Unreal Engine version that fixes this
	// - Modify Unreal Engine's Steam OnlineIdentityInterface source code (if you have engine source)
	// - Use a custom Steam authentication implementation that directly calls GetAuthTicketForWebApi
	//
	UE_LOG_ONLINE(Log, TEXT("GetPlatformAuthToken: Requesting auth token from platform OSS: %s"), *PlatformOSS->GetSubsystemName().ToString());
...
void FUserManagerEOS::LoginViaConnectInterface(const FOnlineAccountCredentials& AccountCredentials)
{
	...
	EEOK_EExternalCredentialType ExternalCredentialType;
	...
	{
		UE_LOG(LogEOK, Error, TEXT("Failed to parse AccountCredentials.Type into BrokenTypeString"));
		ExternalCredentialType = EEOK_EExternalCredentialType::EOK_ECT_EPIC;
	}
	EOS_Connect_Credentials UserCredentials;
	UserCredentials.ApiVersion = EOS_CONNECT_CREDENTIALS_API_LATEST;
	...
	LoginOptions.UserLoginInfo = &LoginInfo;
	int32 LocalUserNum = 0;
	UE_LOG(LogEOK, Log, TEXT("LoginViaConnectInterface called. Login Method: %s"), *AccountCredentials.Type);
	EOS_Connect_Login(EOSSubsystem->ConnectHandle, &LoginOptions, this, &FUserManagerEOS::LoginViaConnectInterfaceCallback);
}

EEOK_EExternalCredentialType FUserManagerEOS::GetExternalCredentialType(const FString& Type)
{
	UEnum* EnumPtr = FindObject<UEnum>(nullptr, TEXT("/Script/OnlineSubsystemEOK.EEOK_EExternalCredentialType"));
	if (!EnumPtr)
	{
		UE_LOG(LogEOK, Error, TEXT("Enum not found in GetExternalCredentialType! Type: %s"), *Type);
		return EEOK_EExternalCredentialType::EOK_ECT_EPIC; // Return a default value or handle error
	}
	int32 EnumValue = EnumPtr->GetValueByName(FName(*Type));
	if (EnumValue == INDEX_NONE)
	{
		UE_LOG(LogEOK, Error, TEXT("Enum value not found in GetExternalCredentialType! Type: %s"), *Type);
		return EEOK_EExternalCredentialType::EOK_ECT_EPIC; // Return a default value or handle error
	}
	return static_cast<EEOK_EExternalCredentialType>(EnumValue);	
}

EEOK_ELoginCredentialType FUserManagerEOS::GetLoginCredentialType(const FString& Type)
{
	UEnum* EnumPtr = FindObject<UEnum>(nullptr, TEXT("/Script/OnlineSubsystemEOK.EEOK_ELoginCredentialType"));
	if (!EnumPtr)
	{
		UE_LOG(LogEOK, Error, TEXT("Enum not found in GetLoginCredentialType! Type: %s"), *Type);
		return EEOK_ELoginCredentialType::EOK_LCT_AccountPortal; // Return a default value or handle error
	}

	int32 EnumValue = EnumPtr->GetValueByName(FName(*Type));
	if (EnumValue == INDEX_NONE)
	{
		UE_LOG(LogEOK, Error, TEXT("Enum value not found in GetLoginCredentialType! Type: %s"), *Type);
		return EEOK_ELoginCredentialType::EOK_LCT_AccountPortal; // Return a default value or handle error
	}
	return static_cast<EEOK_ELoginCredentialType>(EnumValue);	
}
...
bool FUserManagerEOS::Login(int32 LocalUserNum, const FOnlineAccountCredentials& AccountCredentials)
{
	...
	FEOSSettings Settings = UEOKSettings::GetSettings();
	...
	if(AccountCredentials.Type.ParseIntoArray(BrokenTypeString, TEXT("_+_"), true) > 0)
	{
		...
		{
			UE_LOG(LogEOK, Log, TEXT("Login using EOK called. Login Method: %s and UseEas: false"), *BrokenTypeString[1]);
			LoginViaConnectInterface(AccountCredentials);
			return true;
		}
		if(BrokenTypeString[0] == "eas")
		{
			...
			{
				UE_LOG(LogEOK, Log, TEXT("Login using EOK called. Login Method: %s  UseEas: true and ExternalAuth: %s"), *BrokenTypeString[1], *BrokenTypeString[2]);
			}
			else if(BrokenTypeString.Num() > 1)
			{
				UE_LOG(LogEOK, Log, TEXT("Login using EOK called. Login Method: %s  UseEas: true"), *BrokenTypeString[1]);
			}
			LoginViaAuthInterface(LocalUserNum, AccountCredentials);
			return true;
		}
		UE_LOG(LogEOK, Warning, TEXT("Login using EOK called. But the format of the type is not correct. Type: %s"), *AccountCredentials.Type);
	}
	else
	{
		UE_LOG(LogEOK, Warning, TEXT("Login using EOK called. But the format of the type is not correct. Type: %s"), *AccountCredentials.Type);
	...
	{
		/*if(EOKSettings->bUseCountryScope)
		{
	...
	bool bIsAccountPortalLogin = (AccountCredentials.Type == TEXT("accountportal"));
#if ENGINE_MAJOR_VERSION == 5
	FLoginCallback* CallbackObj = new FLoginCallback(AsWeak());
#else
	FLoginCallback* CallbackObj = new FLoginCallback();
#endif
	CallbackObj->CallbackLambda = [this, LocalUserNum, bIsPersistentLogin, bIsAccountPortalLogin](const EOS_Auth_LoginCallbackInfo* Data)
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			// On Android, close the Account Portal browser immediately after successful auth login
			#if PLATFORM_ANDROID
			if (bIsAccountPortalLogin && EOSSubsystem && EOSSubsystem->UIHandle && Data->LocalUserId)
			{
				EOS_UI_HideFriendsOptions HideOptions = {};
				HideOptions.ApiVersion = EOS_UI_HIDEFRIENDS_API_LATEST;
				HideOptions.LocalUserId = Data->LocalUserId;
				EOS_UI_HideFriends(EOSSubsystem->UIHandle, &HideOptions, nullptr, nullptr);
				UE_LOG(LogEOK, Log, TEXT("Account Portal Login Success: Closing browser immediately after auth callback"));
			}
			#endif
...
{
	TArray<FString> BrokenTypeString;
	EEOK_EExternalCredentialType ExternalCredentialType = EEOK_EExternalCredentialType::EOK_ECT_EPIC;
	EEOK_ELoginCredentialType LoginCredentialType = EEOK_ELoginCredentialType::EOK_LCT_AccountPortal;
	if(AccountCredentials.Type.ParseIntoArray(BrokenTypeString, TEXT("_+_"), true) > 0)
	{
		if(BrokenTypeString.Num() > 2)
		{
	...
	EOS_Auth_LoginOptions LoginOptions = { };
	LoginOptions.ApiVersion = EOS_AUTH_LOGIN_API_LATEST;
	UEOKSettings* EOKSettings = GetMutableDefault<UEOKSettings>();
	if(EOKSettings)
	{
		if(EOKSettings->LoginFlags.Contains(EEOK_LoginFlags_LocalForSettings::T_EOS_AS_NoFlags))
		{
			LoginOptions.ScopeFlags = EOS_EAuthScopeFlags::EOS_AS_NoFlags;
		}
		else
		{
			if(EOKSettings->LoginFlags.Contains(EEOK_LoginFlags_LocalForSettings::EOS_AS_Email))
			{
				LoginOptions.ScopeFlags |= EOS_EAuthScopeFlags::EOS_AS_Email;
			}
			if(EOKSettings->LoginFlags.Contains(EEOK_LoginFlags_LocalForSettings::EOS_AS_FriendsList))
			{
				LoginOptions.ScopeFlags |= EOS_EAuthScopeFlags::EOS_AS_FriendsList;
			}
			if(EOKSettings->LoginFlags.Contains(EEOK_LoginFlags_LocalForSettings::EOS_AS_Presence))
			{
				LoginOptions.ScopeFlags |= EOS_EAuthScopeFlags::EOS_AS_Presence;
			}
			if(EOKSettings->LoginFlags.Contains(EEOK_LoginFlags_LocalForSettings::EOS_AS_Country))
			{
				LoginOptions.ScopeFlags |= EOS_EAuthScopeFlags::EOS_AS_Country;
			}
			if(EOKSettings->LoginFlags.Contains(EEOK_LoginFlags_LocalForSettings::EOS_AS_BasicProfile))
			{
	...
	{
		UE_LOG(LogEOK, Warning, TEXT("EOKSettings is null in LoginViaAuthInterface, using default settings"));
		LoginOptions.ScopeFlags = EOS_EAuthScopeFlags::EOS_AS_BasicProfile | EOS_EAuthScopeFlags::EOS_AS_FriendsList | EOS_EAuthScopeFlags::EOS_AS_Presence;
	}
	...
	if(LoginCredentialType == EEOK_ELoginCredentialType::EOK_LCT_PersistentAuth)
	{
		bIsPersistentLogin = true;
	}
	bool bIsAccountPortalLogin = (LoginCredentialType == EEOK_ELoginCredentialType::EOK_LCT_AccountPortal);
	...
	CallbackObj->CallbackLambda = [this, LocalUserNum, bIsPersistentLogin, bIsAccountPortalLogin](const EOS_Auth_LoginCallbackInfo* Data)
	{
		if (Data->ResultCode == EOS_EResult::EOS_Success)
		{
			...
			if (bIsAccountPortalLogin && EOSSubsystem && EOSSubsystem->UIHandle && Data->LocalUserId)
			{
				...
				HideOptions.LocalUserId = Data->LocalUserId;
				EOS_UI_HideFriends(EOSSubsystem->UIHandle, &HideOptions, nullptr, nullptr);
				UE_LOG(LogEOK, Log, TEXT("LoginViaAuthInterface Account Portal: Closing browser immediately after auth callback"));
			}
			#endif
			// Continue the login process by getting the product user id for EAS only
			ConnectLoginEAS(LocalUserNum, Data->LocalUserId);
...
{
	UE_LOG(LogEOK, Log, TEXT("LoginViaConnectInterfaceCallback called. ResultCode: %hs"), EOS_EResult_ToString(Data->ResultCode));
	if(FUserManagerEOS* UserManager = static_cast<FUserManagerEOS*>(Data->ClientData))
	{
		...
		{
			if(UserManager->LocalUserNumToLastLoginCredentials[0].Get().Type == "noeas_+_EOK_ECT_DEVICEID_ACCESS_TOKEN")
			{
...
void FUserManagerEOS::RefreshConnectLogin(int32 LocalUserNum)
{
	...
	const FEOSSettings Settings = UEOKSettings::GetSettings();
	if (true)
	{
...
#if ENGINE_MAJOR_VERSION == 5
typedef TEOKGlobalCallback<EOS_Connect_OnAuthExpirationCallback, EOS_Connect_AuthExpirationCallbackInfo, FUserManagerEOS> FRefreshAuthCallback;
typedef TEOKGlobalCallback<EOS_Presence_OnPresenceChangedCallback, EOS_Presence_PresenceChangedCallbackInfo, FUserManagerEOS> FPresenceChangedCallback;
typedef TEOKGlobalCallback<EOS_Friends_OnFriendsUpdateCallback, EOS_Friends_OnFriendsUpdateInfo, FUserManagerEOS> FFriendsStatusUpdateCallback;
typedef TEOKGlobalCallback<EOS_Auth_OnLoginStatusChangedCallback, EOS_Auth_LoginStatusChangedCallbackInfo, FUserManagerEOS> FLoginStatusChangedCallback;
#else
typedef TEOKGlobalCallback<EOS_Connect_OnAuthExpirationCallback, EOS_Connect_AuthExpirationCallbackInfo> FRefreshAuthCallback;
typedef TEOKGlobalCallback<EOS_Presence_OnPresenceChangedCallback, EOS_Presence_PresenceChangedCallbackInfo> FPresenceChangedCallback;
typedef TEOKGlobalCallback<EOS_Friends_OnFriendsUpdateCallback, EOS_Friends_OnFriendsUpdateInfo> FFriendsStatusUpdateCallback;
typedef TEOKGlobalCallback<EOS_Auth_OnLoginStatusChangedCallback, EOS_Auth_LoginStatusChangedCallbackInfo> FLoginStatusChangedCallback;
#endif
void FUserManagerEOS::FullLoginCallback(int32 LocalUserNum, EOS_EpicAccountId AccountId, EOS_ProductUserId UserId)
{
	...
	// On Android, close the browser overlay after full login completes (Account Portal login)
	#if PLATFORM_ANDROID
	if (EOSSubsystem && EOSSubsystem->UIHandle)
	{
		// Get AccountId from UserNetId if not provided (for Account Portal login)
		EOS_EpicAccountId EpicAccountIdToUse = AccountId;
		if (!EpicAccountIdToUse && UserNetId.IsValid())
		{
			EpicAccountIdToUse = UserNetId->GetEpicAccountId();
		}
		
		if (EpicAccountIdToUse)
		{
			EOS_UI_HideFriendsOptions HideOptions = {};
			HideOptions.ApiVersion = EOS_UI_HIDEFRIENDS_API_LATEST;
			HideOptions.LocalUserId = EpicAccountIdToUse;
			EOS_UI_HideFriends(EOSSubsystem->UIHandle, &HideOptions, nullptr, nullptr);
			UE_LOG(LogEOK, Log, TEXT("FullLoginCallback: Closing browser overlay after successful Account Portal login"));
		}
	}
	#endif
...
void FUserManagerEOS::Tick(float DeltaTime)
{
	...
	if(EnumPtr && EOKSettings)
	{
		if(!bAutoLoginAttempted)
		{
			...
			if(GEngine)
			{
				UE_LOG(LogEOK, Verbose, TEXT("AutoLaunchDevTool: GEngine is valid"));
				World = GEngine->GetWorldContexts()[0].World();
				if(World)
				{
					if(World->WorldType == EWorldType::Editor)
					{
						if(EOKSettings->bAutoLaunchDevTool)
						{
							UE_LOG(LogEOK, Log, TEXT("World is a PIE world. Launching DevTool as auto launch is enabled"));
							LaunchDevTool();
						}
					}
				}
				...
				{
					UE_LOG(LogEOK, Warning, TEXT("World is not valid. Skipping auto launch of DevTool"));
				}
			}
		}
	}
...
bool FUserManagerEOS::AutoLogin(int32 LocalUserNum)
{
	...
	{
		UE_LOG(LogEOK, Log, TEXT("AutoLogin: Unable to AutoLogin user (%d) due to missing auth command line args, attempting to auto login with settings"), LocalUserNum);
		return AutoLoginUsingSettings(LocalUserNum);
	}
	UE_LOG(LogEOK, Log, TEXT("AutoLogin: Attempting to auto login user (%d) with auth command line args"), LocalUserNum);
	FOnlineAccountCredentials Creds(AuthType, LoginId, Password);
	if(AuthType == "exchangecode")
	{
		Creds.Type = "eas_+_EOK_LCT_ExchangeCode_+_EOK_ECT_EPIC";
	...
	{
		Creds.Id = "deviceid";
		Creds.Type = "noeas_+_EOK_ECT_DEVICEID_ACCESS_TOKEN";
	}
	else if(AuthType == "persistentauth")
	{
		Creds.Type = "eas_+_EOK_LCT_PersistentAuth_+_EOK_ECT_EPIC";
		Creds.Token = Password;
		Creds.Id = LoginId;
	}
	else if(AuthType == "accountportal")
	{
		Creds.Type = "eas_+_EOK_LCT_AccountPortal+_EOK_ECT_EPIC";
		Creds.Token = Password;
		Creds.Id = LoginId;
	}
...
void FUserManagerEOS::LaunchDevTool()
{
	...
	TSharedPtr<IPlugin> EOSPlugin = PluginManager.FindPlugin(TEXT("EpicOnlineServices-Kit V2"));
	if(!EOSPlugin.IsValid())
	{
		UE_LOG(LogEOK, Error, TEXT("EpicOnlineServicesKitV2 plugin not found"));
		return;
	}
	FString PluginRoot = EOSPlugin->GetBaseDir();
	auto MainModulePath = FPaths::Combine(*PluginRoot, TEXT("Source/ThirdParty/EOKSDK/Tools/EOS_DevAuthTool-win32-x64-1.2.0"));
...
bool FUserManagerEOS::AutoLoginUsingSettings(int32 LocalUserNum)
{
	if(bAutoLoginAttempted)
	{
		UE_LOG(LogEOK, Warning, TEXT("AutoLoginUsingSettings: Auto login has already been attempted. Skipping auto login"));
		return false;
	}
	UE_LOG(LogEOK, Verbose, TEXT("Checking if the user should be auto logged in"));
	UWorld* World;
	if(GEngine)
	{
		World = GEngine->GetWorldContexts()[0].World();
		if(World)
		{
			...
			{
				UE_LOG(LogEOK, Display, TEXT("World is not a game world. Skipping auto login"));
				bAutoLoginAttempted = true;
				return false;
			}
		}
		...
		{
			UE_LOG(LogEOK, Warning, TEXT("World is not valid. Skipping auto login"));
			bAutoLoginAttempted = true;
			return false;
		}
	}
	...
	{
		UE_LOG(LogEOK, Warning, TEXT("GEngine is not valid. Skipping auto login"));
		bAutoLoginAttempted = true;
		return false;
	}
	UEnum* EnumPtr = FindObject<UEnum>(nullptr, TEXT("/Script/OnlineSubsystemEOK.EEOK_EExternalCredentialType"));
	...
	if(EnumPtr && EOKSettings)
	{
		UE_LOG(LogEOK, Verbose, TEXT("AutoLogin: Enum found for EEOK_EExternalCredentialType. Starting auto login"));
		if(EOKSettings->AutoLoginType == EEOK_AutoLoginType::AutoLogin_None)
		{
			UE_LOG(LogEOK, Display, TEXT("AutoLogin: AutoLoginType is set to None. Skipping auto login"));
			bAutoLoginAttempted = true;
			return false;
		}
		OnLoginCompleteDelegates->AddLambda([this, EOKSettings](int32 LocalUserNum, bool bWasSuccessful, const FUniqueNetId& UserId, const FString& Error)
		{
			if(!bWasSuccessful && bAutoLoginInProgress)
			{
				UE_LOG(LogEOK, Warning, TEXT("AutoLogin: Login failed for user (%d). Attempting to auto login with fallback"), LocalUserNum);
		...
		switch (EOKSettings->AutoLoginType)
		{
		case AutoLogin_None:
			UE_LOG(LogEOK, Warning, TEXT("AutoLogin: AutoLoginType is set to None. Skipping auto login"));
			return true;
			break;
		case AutoLogin_PersistentAuth:
			TempDetails.Type = "eas_+_EOK_LCT_PersistentAuth_+_EOK_ECT_EPIC";
			Login(0,TempDetails);
			bAutoLoginAttempted = true;
		...
		case AutoLogin_DeviceIdLogin:
			TempDetails.Type = "noeas_+_EOK_ECT_DEVICEID_ACCESS_TOKEN";
		...
		case AutoLogin_AccountPortalLogin:
			TempDetails.Type = "eas_+_EOK_LCT_AccountPortal+_EOK_ECT_EPIC";
			bAutoLoginInProgress = true;
			Login(0,TempDetails);
			bAutoLoginAttempted = true;
			return true;
...
#if SUPPORTOCULUSPLATFORM
{
    ...
    GetLoggedInUser_Delegate.BindLambda([&](bool bSuccess, FOvrUserPtr StringPtrParam, FString StringParam)
    {
        if(!bSuccess)
        {
            UE_LOG(LogEOK, Warning, TEXT("AutoLogin: Failed to get logged in user. Skipping auto login"));
            bAutoLoginAttempted = true;
            bAutoLoginInProgress = false;
        ...
        GetUserProof_Delegate.BindLambda([&](bool bSuccess2, FOvrUserProofPtr StringVal, FString StringParam2)
        {
            if(!bSuccess2)
            {
                UE_LOG(LogEOK, Warning, TEXT("AutoLogin: Failed to get user proof. Skipping auto login"));
                bAutoLoginAttempted = true;
                bAutoLoginInProgress = false;
                AutoLoginWithFallback(0);
                return;
            ...
            FOnlineAccountCredentials TempDetails;
            if(EOKSettings->bUse_EAS_ForAutoLogin)
            {
                TempDetails.Type = "eas_+_EOK_LCT_ExternalAuth_+_EOK_ECT_OCULUS_USERID_NONCE";
            }
            else
            {
                TempDetails.Type = "noeas_+_EOK_ECT_OCULUS_USERID_NONCE";
            }
...
#else
UE_LOG(LogEOK, Warning, TEXT("AutoLogin: Oculus login is not supported in this build. Skipping auto login"));
bAutoLoginAttempted = true;
bAutoLoginInProgress = false;
AutoLoginWithFallback(0);
#endif
...
#if ENGINE_MAJOR_VERSION == 5
			GetPlatformAuthToken(0,FOnGetLinkedAccountAuthTokenCompleteDelegate::CreateLambda([this,EOKSettings, WeakThis = AsWeak()](int32 LocalUserNum, bool bWasSuccessful, const FExternalAuthToken& AuthToken)
{
	if (FUserManagerEOSPtr StrongThis = WeakThis.Pin())
#else
			GetPlatformAuthToken(0,FOnGetLinkedAccountAuthTokenCompleteDelegate::CreateLambda([this,EOKSettings](int32 LocalUserNum, bool bWasSuccessful, const FExternalAuthToken& AuthToken)
			{
				if (FUserManagerEOSPtr StrongThis = AsShared())
#endif
					{
					if (!bWasSuccessful || !AuthToken.IsValid())
					{
						UE_LOG(LogEOK, Warning, TEXT("Unable to AutoLogin user (%d) due to an empty platform auth token"), LocalUserNum);
						AutoLoginWithFallback(LocalUserNum);
						bAutoLoginAttempted = true;
						return;
					}
					...
					if(EOKSettings->bUse_EAS_ForAutoLogin)
					{
						TempDetails.Type = "eas_+_EOK_LCT_ExternalAuth_+_EOK_ECT_STEAM_SESSION_TICKET";
					}
					else
					{
						TempDetails.Type = "noeas_+_EOK_ECT_STEAM_SESSION_TICKET";
...
#if WITH_EDITOR
			{
				...
				if (CommandLine.FindChar('=', FoundIndex))
				{
					...
					if (StartIndex != INDEX_NONE)
					{
						...
						UE_LOG(LogEOK, Log, TEXT("AutoLogin: Found GameUserSettingsINI=PIEGameUserSettings. Using LocalUserNum: %d"), FCString::Atoi(*NumberString));
						TempDetails.Type = "eas_+_EOK_LCT_Developer_+_EOK_ECT_EPIC";
						TempDetails.Id = EOKSettings->DeveloperToolUrl;
						int32 TempUserNum = FCString::Atoi(*NumberString) + 1;
						UE_LOG(LogEOK, Log, TEXT("AutoLogin: LocalUserNum: %d"), TempUserNum);
				...
				UE_LOG(LogEOK, Warning, TEXT("AutoLogin: DeveloperTool is enabled but GameUserSettingsINI=PIEGameUserSettings is not found. Skipping auto login"));
				bAutoLoginAttempted = true;
				bAutoLoginInProgress = false;
...
#endif
			default:
			UE_LOG(LogEOK, Warning, TEXT("AutoLogin: Unknown AutoLoginType. Skipping auto login"));
			bAutoLoginAttempted = true;
			return false;
		}
	}
	...
	{
		UE_LOG(LogEOK, Log, TEXT("AutoLogin: Enum not found for EEOK_EExternalCredentialType. Will retry auto login"));
...
{
	UEOKSettings* EOKSettings = GetMutableDefault<UEOKSettings>();
	switch(EOKSettings->FallbackForAutoLoginType)
	{
	case Fallback_None:
		UE_LOG(LogEOK, Warning, TEXT("No fallback for AutoLogin, stopping autolog process"));
		break;
...
bool FUserManagerEOS::GetEpicAccountIdFromProductUserId(const EOS_ProductUserId& ProductUserId, EOS_EpicAccountId& OutEpicAccountId) const
{
	...
	{
		UE_LOG_ONLINE(Verbose, TEXT("[FUserManagerEOS::GetEpicAccountIdFromProductUserId] EOS_Connect_GetProductUserIdMapping not successful for ProductUserId (%s). Finished with EOS_EResult %s"), *EOK_LexToString(ProductUserId), ANSI_TO_TCHAR(EOS_EResult_ToString(Result)));
	}
...
void FUserManagerEOS::ResolveUniqueNetIds(const TArray<EOS_ProductUserId>& ProductUserIds, const FResolveUniqueNetIdsCallback& Callback) const
{
	...
	{
		// Validate LocalUserId before proceeding
		EOS_ProductUserId LocalUserId = EOSSubsystem->UserManager->GetLocalProductUserId(0);
		if (LocalUserId == nullptr)
		{
			UE_LOG_ONLINE(Error, TEXT("[FUserManagerEOS::ResolveUniqueNetIds] Invalid LocalUserId. User may not be logged in. Cannot query ProductUserId mappings."));
			// Return empty results since we can't query
			Callback(ResolvedUniqueNetIds);
			return;
		}
		
		// Filter out invalid ProductUserIds before querying
		TArray<EOS_ProductUserId> ValidProductUserIdsToResolve;
		for (const EOS_ProductUserId& ProductUserId : ProductUserIdsToResolve)
		{
			if (ProductUserId != nullptr)
			{
				ValidProductUserIdsToResolve.Add(ProductUserId);
			}
			else
			{
				UE_LOG_ONLINE(Warning, TEXT("[FUserManagerEOS::ResolveUniqueNetIds] Skipping invalid (nullptr) ProductUserId in query list."));
			}
		}
		
		if (ValidProductUserIdsToResolve.Num() == 0)
		{
			UE_LOG_ONLINE(Warning, TEXT("[FUserManagerEOS::ResolveUniqueNetIds] No valid ProductUserIds to resolve. All ProductUserIds were invalid."));
			Callback(ResolvedUniqueNetIds);
			return;
		}
		...
		QueryProductUserIdMappingsOptions.LocalUserId = LocalUserId;
		QueryProductUserIdMappingsOptions.ProductUserIds = ValidProductUserIdsToResolve.GetData();
		QueryProductUserIdMappingsOptions.ProductUserIdCount = ValidProductUserIdsToResolve.Num();
#if ENGINE_MAJOR_VERSION == 5
...
#endif
		CallbackObj->CallbackLambda = [this, ValidProductUserIdsToResolve, ResolvedUniqueNetIds = MoveTemp(ResolvedUniqueNetIds), Callback](const EOS_Connect_QueryProductUserIdMappingsCallbackInfo* Data) mutable
		{
			if (Data->ResultCode != EOS_EResult::EOS_Success)
			{
				UE_LOG_ONLINE(Verbose, TEXT("[FUserManagerEOS::ResolveUniqueNetIds] EOS_Connect_QueryProductUserIdMappings not successful for user (%s). Finished with EOS_EResult %s."), *EOK_LexToString(Data->LocalUserId), ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
			}

			for (const EOS_ProductUserId& ProductUserId : ValidProductUserIdsToResolve)
			{
				if(ProductUserId == nullptr)
				{
					continue;
...
void FUserManagerEOS::UpdateRemotePlayerProductUserId(EOS_EpicAccountId EpicAccountId, EOS_ProductUserId ProductUserId)
{
	...
	const FString ProductUserIdStr = EOK_LexToString(ProductUserId);
		...
		if (bWasSuccessful)
		{
			...
			for (int32 Index = 0; Index < FriendCount; Index++)
			{
				...
				if (FriendEpicAccountId != nullptr)
				{
					...
					FriendEasIds.Add(EOK_LexToString(FriendEpicAccountId));
				}
			}
...
void FUserManagerEOS::SetPresence(const FUniqueNetId& UserId, const FOnlineUserPresenceStatus& Status, const FOnPresenceTaskCompleteDelegate& Delegate)
{
	...
	{
		UE_LOG_ONLINE(Error, TEXT("EOS_PresenceModification_SetData() failed with result code (%s)"), *EOK_LexToString(SetDataResult));
	}
#if ENGINE_MAJOR_VERSION == 5
	...
	CallbackObj->CallbackLambda = [this, Delegate](const EOS_Presence_SetPresenceCallbackInfo* Data)
	{
		...
		UE_LOG_ONLINE(Error, TEXT("SetPresence() failed with result code (%s)"), *EOK_LexToString(Data->ResultCode));
...
void FUserManagerEOS::QueryPresence(const FUniqueNetId& UserId, const FOnPresenceTaskCompleteDelegate& Delegate)
{
	...
	if (bHasPresence == EOS_FALSE)
	{
		...
		CallbackObj->CallbackLambda = [this, Delegate](const EOS_Presence_QueryPresenceCallbackInfo* Data)
		{
			...
			UE_LOG_ONLINE(Error, TEXT("QueryPresence() for user (%s) failed with result code (%s)"), *TargetUser, *EOK_LexToString(Data->ResultCode));
			Delegate.ExecuteIfBound(*FUniqueNetIdEOS::EmptyId(), false);
		};
	...
	for (const FUniqueNetIdRef& NetId : UserIds)
	{
		...
		if (EOS_EpicAccountId_IsValid(AccountId) == EOS_TRUE)
		{
			...
			else
			{
				...
				UserEasIdsNeedingExternalMappings.Add(EOK_LexToString(AccountId));
...
bool FUserManagerEOS::QueryUserIdMapping(const FUniqueNetId& UserId, const FString& DisplayNameOrEmail, const FOnQueryUserMappingComplete& Delegate)
{
	...
	CallbackObj->CallbackLambda = [LocalUserNum, DisplayNameOrEmail, this, Delegate](const EOS_UserInfo_QueryUserInfoByDisplayNameCallbackInfo* Data)
	{
		...
		if (bWasSuccessful)
		{
			const FString NetIdStr = EOK_LexToString(Data->TargetUserId);
