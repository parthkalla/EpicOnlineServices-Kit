...
#else
	: SessionId(FUniqueNetIdString::Create(TEXT("INVALID"), TEXT("EOK")))
#endif
...
#if ENGINE_MAJOR_VERSION == 5
typedef TEOKGlobalCallback<EOS_Sessions_OnSessionInviteReceivedCallback, EOS_Sessions_SessionInviteReceivedCallbackInfo, FOnlineSessionEOS> FSessionInviteReceivedCallback;
typedef TEOKGlobalCallback<EOS_Sessions_OnSessionInviteAcceptedCallback, EOS_Sessions_SessionInviteAcceptedCallbackInfo, FOnlineSessionEOS> FSessionInviteAcceptedCallback;
...
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyUpdateReceivedCallback, EOS_Lobby_LobbyUpdateReceivedCallbackInfo, FOnlineSessionEOS> FLobbyUpdateReceivedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyMemberUpdateReceivedCallback, EOS_Lobby_LobbyMemberUpdateReceivedCallbackInfo, FOnlineSessionEOS> FLobbyMemberUpdateReceivedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyMemberStatusReceivedCallback, EOS_Lobby_LobbyMemberStatusReceivedCallbackInfo, FOnlineSessionEOS> FLobbyMemberStatusReceivedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyInviteAcceptedCallback, EOS_Lobby_LobbyInviteAcceptedCallbackInfo, FOnlineSessionEOS> FLobbyInviteAcceptedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnJoinLobbyAcceptedCallback, EOS_Lobby_JoinLobbyAcceptedCallbackInfo, FOnlineSessionEOS> FJoinLobbyAcceptedCallback;
#if PLATFORM_WINDOWS
typedef TEOKGlobalCallback<EOS_Lobby_OnLeaveLobbyRequestedCallback, EOS_Lobby_LeaveLobbyRequestedCallbackInfo, FOnlineSessionEOS> FLeaveLobbyRequestCallback;
#endif
#else
typedef TEOKGlobalCallback<EOS_Sessions_OnSessionInviteReceivedCallback, EOS_Sessions_SessionInviteReceivedCallbackInfo> FSessionInviteReceivedCallback;
typedef TEOKGlobalCallback<EOS_Sessions_OnSessionInviteAcceptedCallback, EOS_Sessions_SessionInviteAcceptedCallbackInfo> FSessionInviteAcceptedCallback;
...
// Lobby notification callbacks
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyUpdateReceivedCallback, EOS_Lobby_LobbyUpdateReceivedCallbackInfo> FLobbyUpdateReceivedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyMemberUpdateReceivedCallback, EOS_Lobby_LobbyMemberUpdateReceivedCallbackInfo> FLobbyMemberUpdateReceivedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyMemberStatusReceivedCallback, EOS_Lobby_LobbyMemberStatusReceivedCallbackInfo> FLobbyMemberStatusReceivedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnLobbyInviteAcceptedCallback, EOS_Lobby_LobbyInviteAcceptedCallbackInfo> FLobbyInviteAcceptedCallback;
typedef TEOKGlobalCallback<EOS_Lobby_OnJoinLobbyAcceptedCallback, EOS_Lobby_JoinLobbyAcceptedCallbackInfo> FJoinLobbyAcceptedCallback;
#if PLATFORM_WINDOWS
typedef TEOKGlobalCallback<EOS_Lobby_OnLeaveLobbyRequestedCallback, EOS_Lobby_LeaveLobbyRequestedCallbackInfo> FLeaveLobbyRequestCallback;
#endif
#endif
...
void FOnlineSessionEOS::Init(const FString& InBucketId)
{
	...
	SessionInviteAcceptedCallbackObj->CallbackLambda = [this](const EOS_Sessions_SessionInviteAcceptedCallbackInfo* Data)
	{
		...
		if (!NetId.IsValid())
		{
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Cannot accept invite due to unknown user (%s)"), *EOK_LexToString(Data->LocalUserId));
	...
	{
		if (!GConfig->GetBool(TEXT("/Script/OnlineSubsystemEOK.NetDriverEOK"), TEXT("bIsUsingP2PSockets"), bIsUsingP2PSockets, GEngineIni))
		{
...
void FOnlineSessionEOS::OnMemberStatusReceived(const EOS_LobbyId& LobbyId, const EOS_ProductUserId& TargetUserId, EOS_ELobbyMemberStatus CurrentStatus)
{
	...
	if (Session)
	{
		...
		case EOS_ELobbyMemberStatus::EOS_LMS_KICKED:
			{
				// Check if the kicked player is the local player
				EOS_ProductUserId LocalProductUserId = EOSSubsystem->UserManager->GetLocalProductUserId();
				bool bIsLocalPlayerKicked = (LocalProductUserId != nullptr && 
					EOS_ProductUserId_IsValid(LocalProductUserId) && 
					EOS_ProductUserId_IsValid(TargetUserId) &&
					LocalProductUserId == TargetUserId);
				
				EOSSubsystem->UserManager->ResolveUniqueNetId(TargetUserId, [this, LobbyNetId, bIsLocalPlayerKicked, LobbyId](FUniqueNetIdEOSRef ResolvedUniqueNetId)
					{
						FNamedOnlineSession* Session = GetNamedSessionFromLobbyId(*LobbyNetId);
						if (Session)
						{
							...
							// If the local player was kicked, force them to leave the lobby and disconnect
							if (bIsLocalPlayerKicked)
							{
								UE_LOG_ONLINE_SESSION(Warning, TEXT("[FOnlineSessionEOS::OnMemberStatusReceived] Local player was kicked from lobby. Forcing disconnect and lobby leave."));
								...
								LeaveOptions.LocalUserId = EOSSubsystem->UserManager->GetLocalProductUserId();
...
#endif
								LeaveCallbackObj->CallbackLambda = [this, SessionName](const EOS_Lobby_LeaveLobbyCallbackInfo* Data)
								{
									...
									if (LobbySession)
									{
										...
										{
											UE_LOG_ONLINE_SESSION(Log, TEXT("[FOnlineSessionEOS::OnMemberStatusReceived] Successfully left lobby after being kicked."));
										}
										else
										{
											UE_LOG_ONLINE_SESSION(Warning, TEXT("[FOnlineSessionEOS::OnMemberStatusReceived] Failed to leave lobby after kick. EOS_EResult: %s"), 
												ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
										}
										
										// Disconnect from game session and travel to a different level
											...
											if (GEngine && GEngine->GetWorldContexts().Num() > 0)
											{
												...
												if (World)
												{
													...
													if (APlayerController* PC = World->GetFirstPlayerController())
													{
														...
														{
															PC->ClientTravel(TEXT("?closed"), ETravelType::TRAVEL_Absolute);
														}
													}
													
													// Travel to return level or default level
													if (UEOKSettings* EOKSettings = GetMutableDefault<UEOKSettings>())
													{
														FName LevelName = FName(*EOKSettings->ReturnLevelName);
														if (!LevelName.IsNone())
														{
															UE_LOG_ONLINE_SESSION(Log, TEXT("[FOnlineSessionEOS::OnMemberStatusReceived] Traveling to return level: %s"), *LevelName.ToString());
															UGameplayStatics::OpenLevel(World, LevelName, true, "");
														}
														else
														{
															UE_LOG_ONLINE_SESSION(Warning, TEXT("[FOnlineSessionEOS::OnMemberStatusReceived] ReturnLevelName is not set. Cannot travel after kick."));
														}
													}
												}
											}
...
void FOnlineSessionEOS::OnLobbyInviteAccepted(const char* InviteId, const EOS_ProductUserId& LocalUserId, const EOS_ProductUserId& TargetUserId)
{
	...
	if (!NetId.IsValid())
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("[FOnlineSessionEOS::OnLobbyInviteAccepted] Cannot accept lobby invite due to unknown user (%s)"), *EOK_LexToString(LocalUserId));
		TriggerOnSessionUserInviteAcceptedDelegates(false, 0, NetId, FOnlineSessionSearchResult());
		return;
	}
	int32 LocalUserNum = EOSSubsystem->UserManager->GetLocalUserNumFromUniqueNetId(*NetId);
...
void FOnlineSessionEOS::OnJoinLobbyAccepted(const EOS_ProductUserId& LocalUserId, const EOS_UI_EventId& UiEventId)
{
	...
	if (!NetId.IsValid())
	{
		UE_LOG_ONLINE_SESSION(Warning, TEXT("[FOnlineSessionEOS::OnJoinLobbyAccepted] Cannot join lobby due to unknown user (%s)"), *EOK_LexToString(LocalUserId));
		TriggerOnSessionUserInviteAcceptedDelegates(false, 0, NetId, FOnlineSessionSearchResult());
		return;
	}
	int32 LocalUserNum = EOSSubsystem->UserManager->GetLocalUserNumFromUniqueNetId(*NetId);
...
void FOnlineSessionEOS::SetPermissionLevel(EOS_HSessionModification SessionModHandle, FNamedOnlineSession* Session)
{
	...
	{
		UE_LOG_ONLINE_SESSION(Error, TEXT("EOS_SessionModification_SetPermissionLevel() failed with EOS result code (%s)"), *EOK_LexToString(ResultCode));
	}
}
...
void FOnlineSessionEOS::SetMaxPlayers(EOS_HSessionModification SessionModHandle, FNamedOnlineSession* Session)
{
	...
	if (ResultCode != EOS_EResult::EOS_Success)
	{
		UE_LOG_ONLINE_SESSION(Error, TEXT("EOS_SessionModification_SetMaxPlayers() failed with EOS result code (%s)"), *EOK_LexToString(ResultCode));
	}
}
...
void FOnlineSessionEOS::SetInvitesAllowed(EOS_HSessionModification SessionModHandle, FNamedOnlineSession* Session)
{
	...
	if (ResultCode != EOS_EResult::EOS_Success)
	{
		UE_LOG_ONLINE_SESSION(Error, TEXT("EOS_SessionModification_SetInvitesAllowed() failed with EOS result code (%s)"), *EOK_LexToString(ResultCode));
	}
}
...
void FOnlineSessionEOS::SetJoinInProgress(EOS_HSessionModification SessionModHandle, FNamedOnlineSession* Session)
{
	...
	if (ResultCode != EOS_EResult::EOS_Success)
	{
		UE_LOG_ONLINE_SESSION(Error, TEXT("EOS_SessionModification_SetJoinInProgressAllowed() failed with EOS result code (%s)"), *EOK_LexToString(ResultCode));
	}
}
...
void FOnlineSessionEOS::AddAttribute(EOS_HSessionModification SessionModHandle, const EOS_Sessions_AttributeData* Attribute)
{
	...
	if (ResultCode != EOS_EResult::EOS_Success)
	{
		UE_LOG_ONLINE_SESSION(Error, TEXT("EOS_SessionModification_AddAttribute() failed for attribute name (%s) with EOS result code (%s)"), *FString(Attribute->Key), *EOK_LexToString(ResultCode));
	}
}
...
uint32 FOnlineSessionEOS::CreateEOSSession(int32 HostingPlayerNum, FNamedOnlineSession* Session)
{
	...
	if (!bIsDedicatedServer && bIsUsingP2PSockets)
	{
		...
		FName NetDriverName = GetDefault<UNetDriverEOK>()->NetDriverName;
		FInternetAddrEOS TempAddr(EOK_LexToString(Options.LocalUserId), NetDriverName.ToString(), GetTypeHash(NetDriverName.ToString()));
...
void FOnlineSessionEOS::AddSearchResult(EOS_HSessionDetails SessionHandle, const TSharedRef<FOnlineSessionSearch>& SearchSettings)
{
	...
	{
		// Check if this session belongs to the current user by checking OwningUserId attribute
		EOS_ProductUserId LocalProductUserId = EOSSubsystem->UserManager->GetLocalProductUserId(0);
		FUniqueNetIdPtr LocalUniqueNetId = EOSSubsystem->UserManager->GetLocalUniqueNetIdEOS(0);
		
		// Get session attribute count to check for OwningUserId
		EOS_SessionDetails_GetSessionAttributeCountOptions CountOptions = { };
		CountOptions.ApiVersion = EOS_SESSIONDETAILS_GETSESSIONATTRIBUTECOUNT_API_LATEST;
		int32 AttributeCount = EOS_SessionDetails_GetSessionAttributeCount(SessionHandle, &CountOptions);
		
		bool bIsOwnSession = false;
		for (int32 AttrIndex = 0; AttrIndex < AttributeCount; AttrIndex++)
		{
			...
			EOS_EResult AttrResult = EOS_SessionDetails_CopySessionAttributeByIndex(SessionHandle, &AttrOptions, &Attribute);
			if (AttrResult == EOS_EResult::EOS_Success && Attribute != nullptr)
			{
				FString Key = Attribute->Data->Key;
				if (Key == TEXT("OwningUserId"))
				{
					FUniqueNetIdPtr SessionOwnerId = FUniqueNetIdEOSRegistry::FindOrAdd(UTF8_TO_TCHAR(Attribute->Data->Value.AsUtf8));
					if (LocalUniqueNetId.IsValid() && SessionOwnerId.IsValid() && *LocalUniqueNetId == *SessionOwnerId)
					{
						UE_LOG_ONLINE_SESSION(Verbose, TEXT("[FOnlineSessionEOS::AddSearchResult] Excluding own session (SessionId: %hs) from search results"), SessionInfo->SessionId);
						bIsOwnSession = true;
						EOS_SessionDetails_Attribute_Release(Attribute);
						break;
					}
		...
		// Also check for matchmaking sessions - sessions with specific attributes that indicate matchmaking
		// For now, we'll skip this check as matchmaking detection would require specific attribute patterns
...
bool FOnlineSessionEOS::JoinSession(int32 PlayerNum, FName SessionName, const FOnlineSessionSearchResult& DesiredSession)
{
	...
	{
		// Check if the existing session is in a valid state or if we should clean it up
		// Clean up sessions that are in invalid, ended, or intermediate states that might block joining
		if (Session->SessionState == EOnlineSessionState::NoSession || 
			...
			Session->SessionState == EOnlineSessionState::Starting)
		{
			// Session is in an invalid/ended/intermediate state, clean it up and allow rejoin
			// This handles cases where a previous join attempt failed or was interrupted
			UE_LOG_ONLINE_SESSION(Verbose, TEXT("Session (%s) exists but is in state (%d). Cleaning up and allowing rejoin."), 
				*SessionName.ToString(), (int32)Session->SessionState);
			RemoveNamedSession(SessionName);
			
			// Retry joining with the cleaned up session
			return JoinSession(PlayerNum, SessionName, DesiredSession);
		}
		else if (Session->SessionState == EOnlineSessionState::InProgress || 
				 Session->SessionState == EOnlineSessionState::Ending)
		{
			// Session is actively in progress or ending - check if we should still allow join
			// For now, we'll block to prevent joining an active session twice
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Session (%s) already exists and is active (state: %d), can't join twice"), 
				*SessionName.ToString(), (int32)Session->SessionState);
		}
		else
		{
			// Unknown state - be safe and block
			UE_LOG_ONLINE_SESSION(Warning, TEXT("Session (%s) already exists in unknown state (%d), can't join twice"), 
				*SessionName.ToString(), (int32)Session->SessionState);
		}
	}
...
void FOnlineSessionEOS::RemovePlayerFromSession(int32 LocalUserNum, FName SessionName, const FUniqueNetId& TargetPlayerId)
{
	...
	if (Session)
	{
		...
		if (LocalUserId == nullptr)
		{
			UE_LOG_ONLINE_SESSION(Error, TEXT("[FOnlineSessionEOS::RemovePlayerFromSession] Invalid LocalUserId for player %d. User may not be logged in."), LocalUserNum);
			return;
		}
		
		const FUniqueNetIdEOS& TargetPlayerEOSId = FUniqueNetIdEOS::Cast(TargetPlayerId);
		EOS_ProductUserId TargetUserId = TargetPlayerEOSId.GetProductUserId();
		
		// Validate TargetUserId
		if (TargetUserId == nullptr || EOS_ProductUserId_IsValid(TargetUserId) == EOS_FALSE)
		{
			UE_LOG_ONLINE_SESSION(Error, TEXT("[FOnlineSessionEOS::RemovePlayerFromSession] Invalid TargetUserId. Cannot kick player."));
			return;
		}
		...
		KickMemberOptions.TargetUserId = TargetUserId;
#if ENGINE_MAJOR_VERSION == 5
		...
		CallbackObj->CallbackLambda = [this](const EOS_Lobby_KickMemberCallbackInfo* Data)
		{
			if (Data->ResultCode == EOS_EResult::EOS_Success)
			{
				UE_LOG_ONLINE_SESSION(Log, TEXT("[FOnlineSessionEOS::RemovePlayerFromSession] KickMember finished successfully for lobby %hs. TargetUserId will be removed from session."), Data->LobbyId);
			}
			else
			{
				UE_LOG_ONLINE_SESSION(Error, TEXT("[FOnlineSessionEOS::RemovePlayerFromSession] KickMember FAILED for lobby %hs. EOS_EResult: %s. TargetUserId may still be in the lobby."), 
					Data->LobbyId, ANSI_TO_TCHAR(EOS_EResult_ToString(Data->ResultCode)));
			}
		};
...
uint32 FOnlineSessionEOS::CreateLobbySession(int32 HostingPlayerNum, FNamedOnlineSession* Session)
{
	...
	CreateLobbyOptions.ApiVersion = 9; // Using version 9 instead of EOS_LOBBY_CREATELOBBY_API_LATEST (10) for compatibility with SDK runtime
	...
	// Log invite setting for debugging
	if (!CreateLobbyOptions.bAllowInvites)
	{
		UE_LOG_ONLINE_SESSION(Verbose, TEXT("[FOnlineSessionEOS::CreateLobbySession] bAllowInvites is false. Social overlay invites will not work for this lobby."));
	}
	else
	{
		UE_LOG_ONLINE_SESSION(Verbose, TEXT("[FOnlineSessionEOS::CreateLobbySession] bAllowInvites is true. Social overlay invites are enabled."));
	}
	...
	CallbackObj->CallbackLambda = [this, SessionName, LocalProductUserId, LocalUserNetId](const EOS_Lobby_CreateLobbyCallbackInfo* Data)
	{
		...
		if (Session)
		{
			...
			if (bWasSuccessful)
			{
				...
				// Because some platforms remap ports, we will use the ID of the name of the net driver to be our port instead
				FName NetDriverName = GetDefault<UNetDriverEOK>()->NetDriverName;
				FInternetAddrEOS TempAddr(EOK_LexToString(LocalProductUserId), SessionName.ToString(), FURL::UrlConfig.DefaultPort);
...
uint32 FOnlineSessionEOS::JoinLobbySession(int32 PlayerNum, FNamedOnlineSession* Session, const FOnlineSession* SearchSession)
{
	...
	if (Session->SessionInfo.IsValid())
	{
		...
		if (EOSSessionInfo->SessionId->IsValid())
		{
			...
			// Validate ProductUserId before proceeding
			EOS_ProductUserId LocalProductUserId = EOSSubsystem->UserManager->GetLocalProductUserId(PlayerNum);
			if (LocalProductUserId == nullptr)
			{
				UE_LOG_ONLINE_SESSION(Error, TEXT("[FOnlineSessionEOS::JoinLobbySession] Invalid ProductUserId for player %d. User may not be logged in."), PlayerNum);
			...
			// The SDK headers define LATEST as 5, but the library binary doesn't support it yet
			JoinLobbyOptions.ApiVersion = 4; // EOS_LOBBY_JOINLOBBY_API_LATEST is 5, but SDK only supports up to 4
...
void FOnlineSessionEOS::AddLobbySearchResult(const TSharedRef<FLobbyDetailsEOS>& LobbyDetails, const TSharedRef<FOnlineSessionSearch>& SearchSettings, const FOnCopyLobbyDataCompleteCallback& Callback)
{
	...
	if (CopyResult == EOS_EResult::EOS_Success)
	{
		// Get the current user's ProductUserId to check if this is their own lobby
		EOS_ProductUserId LocalProductUserId = EOSSubsystem->UserManager->GetLocalProductUserId(0);
		
		// Check if this lobby is owned by the current user
		if (LocalProductUserId != nullptr && EOS_ProductUserId_IsValid(LocalProductUserId) && 
			EOS_ProductUserId_IsValid(LobbyDetailsInfo->LobbyOwnerUserId) &&
			LocalProductUserId == LobbyDetailsInfo->LobbyOwnerUserId)
		{
			UE_LOG_ONLINE_SESSION(Verbose, TEXT("[FOnlineSessionEOS::AddLobbySearchResult] Excluding own lobby (LobbyId: %hs) from search results"), LobbyDetailsInfo->LobbyId);
			EOS_LobbyDetails_Info_Release(LobbyDetailsInfo);
			Callback(false);
			return;
		}
		
		// Check if the user is already a member of this lobby
		EOS_LobbyDetails_GetMemberCountOptions CountOptions = { };
		CountOptions.ApiVersion = EOS_LOBBYDETAILS_GETMEMBERCOUNT_API_LATEST;
		int32 MemberCount = EOS_LobbyDetails_GetMemberCount(LobbyDetails->LobbyDetailsHandle, &CountOptions);
		...
		for (int32 Index = 0; Index < MemberCount; Index++)
		{
			...
			EOS_ProductUserId MemberUserId = EOS_LobbyDetails_GetMemberByIndex(LobbyDetails->LobbyDetailsHandle, &GetMemberByIndexOptions);
			if (LocalProductUserId != nullptr && EOS_ProductUserId_IsValid(LocalProductUserId) && 
				EOS_ProductUserId_IsValid(MemberUserId) &&
				LocalProductUserId == MemberUserId)
			{
				UE_LOG_ONLINE_SESSION(Verbose, TEXT("[FOnlineSessionEOS::AddLobbySearchResult] Excluding lobby (LobbyId: %hs) - user is already a member"), LobbyDetailsInfo->LobbyId);
				EOS_LobbyDetails_Info_Release(LobbyDetailsInfo);
				Callback(false);
				return;
			}
		...
		// Because some platforms remap ports, we will use the ID of the name of the net driver to be our port instead
		FName NetDriverName = GetDefault<UNetDriverEOK>()->NetDriverName;
		FInternetAddrEOS TempAddr(EOK_LexToString(LobbyDetailsInfo->LobbyOwnerUserId), NetDriverName.ToString(), GetTypeHash(NetDriverName.ToString()));
		FString HostAddr = TempAddr.ToString(true);
...
void FOnlineSessionEOS::CopyLobbyData(const TSharedRef<FLobbyDetailsEOS>& LobbyDetails, EOS_LobbyDetails_Info* LobbyDetailsInfo, FOnlineSession& OutSession, const FOnCopyLobbyDataCompleteCallback& Callback)
{
	...
	// Only add owner if it's valid
	if (OwnerID != nullptr && EOS_ProductUserId_IsValid(OwnerID) == EOS_TRUE)
	{
	...
	for (int32 Index = 0; Index < Count; Index++)
	{
		...
		// Skip invalid user IDs, owner (already added), or duplicates
		if(TargetUserId == nullptr || 
		   TargetUserId == EOS_ProductUserId() ||
		   EOS_ProductUserId_IsValid(TargetUserId) == EOS_FALSE ||
		   TargetUserId == OwnerID ||
		   TargetUserIds.Contains(TargetUserId))
		{
			continue;
		}
