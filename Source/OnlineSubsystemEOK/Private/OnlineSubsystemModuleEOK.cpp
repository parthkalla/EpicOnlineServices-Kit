...
#include "OnlineSubsystemEOKModule.h"
...
#define LOCTEXT_NAMESPACE "EOK"

IMPLEMENT_MODULE(FOnlineSubsystemEOKModule, OnlineSubsystemEOK);
...
void FOnlineSubsystemEOKModule::StartupModule()
{
	...
	OSS.RegisterPlatformService("EOK", EOSFactory);
...
#if WITH_EDITOR
	UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(this, &FOnlineSubsystemEOKModule::RegisterMenus));
#endif
#if WITH_EDITOR
	FCoreDelegates::OnPostEngineInit.AddRaw(this, &FOnlineSubsystemEOKModule::OnPostEngineInit);
	FCoreDelegates::OnPreExit.AddRaw(this, &FOnlineSubsystemEOKModule::OnPreExit);
#endif
}

#if WITH_EDITOR
void FOnlineSubsystemEOKModule::OnPostEngineInit()
{
	if (ISettingsModule* SettingsModule = FModuleManager::GetModulePtr<ISettingsModule>("Settings"))
	{
		SettingsModule->RegisterSettings("Project", "Game", "Online Subsystem EOK",
			LOCTEXT("OSSEOSSettingsName", "Epic Online Services-Kit V2"),
			LOCTEXT("OSSEOSSettingsDescription", "Take control of your Epic Online Services-Kit V2 settings here. Explore the options available or refer to the documentation for further information on how to customize your settings."),
			GetMutableDefault<UEOKSettings>());
	}
}
#endif

#if WITH_EDITOR
void FOnlineSubsystemEOKModule::OnPreExit()
{
	if (ISettingsModule* SettingsModule = FModuleManager::GetModulePtr<ISettingsModule>("Settings"))
	{
		SettingsModule->UnregisterSettings("Project", "Game", "Online Subsystem EOK");
	}
}
#endif

void FOnlineSubsystemEOKModule::ShutdownModule()
{
	if (IsRunningCommandlet())
	{
		return;
	...
	FOnlineSubsystemModule& OSS = FModuleManager::GetModuleChecked<FOnlineSubsystemModule>("OnlineSubsystem");
	OSS.UnregisterPlatformService("EOK");
...
void FOnlineSubsystemEOKModule::PluginButtonClicked()
{
#if WITH_EDITOR
	FEOSSettings EOSSettings = UEOKSettings::GetSettings();
	UEOKSettings* Settings = GetMutableDefault<UEOKSettings>();
	FString ProductName = Settings->ProductName;
	FString OrganizationId = Settings->OrganizationName;
...
void FOnlineSubsystemEOKModule::RegisterMenus()
{
#if WITH_EDITOR
...
void FOnlineSubsystemEOKModule::ConfigureOnlineSubsystemEOK()
{
    ...
    if (FFileHelper::LoadFileToString(EngineIniText, *EngineIniPath))
    {
        ...
        if (!EngineIniText.Contains(TEXT("\n[/Script/OnlineSubsystemEOK.NetDriverEOK]")))
        {
        	FString Comment = TEXT(";EOK Comment: You do not need to worry about this setting as we dynamically set it in Travel URL depending upon if we are using Listen Server or Dedicated Server\n");
        ...
        if (bConfigChanged)
        {
            ...
            {
                UE_LOG(LogTemp, Warning, TEXT("OnlineSubsystemEOK configuration added/updated in DefaultEngine.ini"));
            }
            else
            {
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("OnlineSubsystemEOK configuration already exists in DefaultEngine.ini"));
        }
    }
    else
    {
