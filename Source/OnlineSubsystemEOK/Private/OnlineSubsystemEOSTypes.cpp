...
{
	static FName NAME_Eos(TEXT("EOK"));
	return NAME_Eos;
}
...
{
	if(EOK_LexToString(EpicAccountId).IsEmpty())
	{
		return EOK_LexToString(ProductUserId);
	}
	return EOK_LexToString(EpicAccountId) + EOS_ID_SEPARATOR + EOK_LexToString(ProductUserId);
}
...
FString FUniqueNetIdEOS::ToDebugString() const
{
	if (IsValid())
	{
		const FString EpicAccountIdStr = EOK_LexToString(EpicAccountId);
...
{
	HexToBytes(EOK_LexToString(EpicAccountId), RawBytes);
	HexToBytes(EOK_LexToString(ProductUserId), RawBytes + ID_HALF_BYTE_SIZE);
}
