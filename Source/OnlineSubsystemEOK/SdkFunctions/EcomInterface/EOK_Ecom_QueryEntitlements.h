...
#include "EOK_Ecom_QueryEntitlements.generated.h"

USTRUCT(BlueprintType)
struct FEOK_Ecom_QueryEntitlementsOptions
{
	GENERATED_BODY()
	...
	FEOK_Ecom_QueryEntitlementsOptions()
	{
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FEOK_Ecom_QueryEntitlementsCallback, const FEOK_EpicAccountId&, LocalUserId, const TEnumAsByte<EEOK_Result>&, Result);

UCLASS()
class OnlineSubsystemEOK_API UEOK_Ecom_QueryEntitlements : public UBlueprintAsyncActionBase
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Epic Online Services-Kit V2 | SDK Functions | Ecom Interface", DisplayName="EOS_Ecom_QueryEntitlements")
	static UEOK_Ecom_QueryEntitlements* EOK_Ecom_QueryEntitlements(FEOK_Ecom_QueryEntitlementsOptions QueryEntitlementsOptions);

	UPROPERTY(BlueprintAssignable)
	FEOK_Ecom_QueryEntitlementsCallback OnCallback;
private:
	static void EOS_CALL OnQueryEntitlementsCallback(const EOS_Ecom_QueryEntitlementsCallbackInfo* Data);
	virtual void Activate() override;
	FEOK_Ecom_QueryEntitlementsOptions Local_QueryEntitlementsOptions;
};
