...
#include "EOK_Lobby_CreateLobby.generated.h"

//Input parameters for the EOS_Lobby_CreateLobby function.
USTRUCT(BlueprintType)
struct FEOK_Lobby_CreateLobbyOptions
{
	GENERATED_BODY()

	//The Product User ID of the local user creating the lobby; this user will automatically join the lobby as its owner
	UPROPERTY(BlueprintReadWrite, Category = "Epic Online Services-Kit V2 | SDK Functions | Lobby Interface")
	...
	UPROPERTY(BlueprintReadWrite, Category = "Epic Online Services-Kit V2 | SDK Functions | Lobby Interface")
	FEOK_Lobby_LocalRTCOptions LocalRTCOptions;
	...
	FEOK_Lobby_CreateLobbyOptions()
	{
		MaxLobbyMembers = 0;
		PermissionLevel = EEOK_ELobbyPermissionLevel::EOK_LPL_InviteOnly;
	...
	EOS_Lobby_CreateLobbyOptions GetCreateLobbyOptions()
	{
		...
		Options.ApiVersion = 9; // Using version 9 instead of EOS_LOBBY_CREATELOBBY_API_LATEST (10) for compatibility with SDK runtime
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FEOK_Lobby_CreateLobbyComplete, const TEnumAsByte<EEOK_Result>&, Result, const FEOK_LobbyId&, LobbyId);

UCLASS()
class OnlineSubsystemEOK_API UEOK_Lobby_CreateLobby : public UBlueprintAsyncActionBase
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Epic Online Services-Kit V2 | SDK Functions | Lobby Interface", DisplayName="EOS_Lobby_CreateLobby")
	static UEOK_Lobby_CreateLobby* EOK_Lobby_CreateLobby(FEOK_Lobby_CreateLobbyOptions CreateLobbyOptions);

	UPROPERTY(BlueprintAssignable, Category = "Epic Online Services-Kit V2 | SDK Functions | Lobby Interface")
	FEOK_Lobby_CreateLobbyComplete OnCallback;
	
private:
	FEOK_Lobby_CreateLobbyOptions Var_CreateLobbyOptions;
