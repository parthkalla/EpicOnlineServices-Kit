...
#include "EOK_Lobby_JoinLobby.generated.h"

USTRUCT(BlueprintType)
struct FEOK_Lobby_JoinLobbyOptions
{
	GENERATED_BODY()
	...
	FEOK_HLobbyDetails LobbyDetailsHandle;
...
DECLARE_DYNAMIC_MULTICAST_DELEGATE_TwoParams(FEOK_Lobby_JoinLobbyDelegate, const TEnumAsByte<EEOK_Result>&, Result, const FEOK_LobbyId&, LobbyId);

UCLASS()
class OnlineSubsystemEOK_API UEOK_Lobby_JoinLobby : public UBlueprintAsyncActionBase
{
	GENERATED_BODY()
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Epic Online Services-Kit V2 | SDK Functions | Lobby Interface", DisplayName="EOS_Lobby_JoinLobby")
	static UEOK_Lobby_JoinLobby* EOK_Lobby_JoinLobby(FEOK_Lobby_JoinLobbyOptions Options);

	UPROPERTY(BlueprintAssignable, Category = "Epic Online Services-Kit V2 | SDK Functions | Lobby Interface")
	FEOK_Lobby_JoinLobbyDelegate OnCallback;
	
private:
	static void EOS_CALL OnJoinLobbyComplete(const EOS_Lobby_JoinLobbyCallbackInfo* Data);
	virtual void Activate() override;
	FEOK_Lobby_JoinLobbyOptions Var_Options;
};
