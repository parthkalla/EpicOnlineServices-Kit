...
#include "EOK_CreateDeviceId_AsyncFunction.generated.h"


/** The result of a create device id request
 * @param Result The result of the create device id request
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_OneParam(FOnCreateDeviceIdComplete, const TEnumAsByte<EEOK_Result>&, Result);
UCLASS()
class OnlineSubsystemEOK_API UEOK_CreateDeviceId_AsyncFunction : public UBlueprintAsyncActionBase
{
	GENERATED_BODY()
...
public:
	...
	*/
	UFUNCTION(BlueprintCallable, Category = "Epic Online Services-Kit V2 | SDK Functions | Connect Interface", DisplayName="EOS_Connect_CreateDeviceId")
	static UEOK_CreateDeviceId_AsyncFunction* CreateDeviceId(FString DeviceModel);
