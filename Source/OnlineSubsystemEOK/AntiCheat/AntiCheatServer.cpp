...
bool UAntiCheatServer::RegisterAntiCheatServer(FString ServerName, FString ClientProductID)
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
		{
			if(!EOSRef->AntiCheatServerHandle)
			{
				UE_LOG(LogEOK, Warning, TEXT("RegisterAntiCheatServer-> AntiCheatServerHandle is null"));
				return false;
			}
			{
			...
			}
			{
				...
				{
					UE_LOG(LogEOK, Log, TEXT("RegisterAntiCheatServer-> Success"));
					return true;
				}
				UE_LOG(LogEOK, Log, TEXT("RegisterAntiCheatServer-> %hs"), EOS_EResult_ToString(Result));
				return false;
			}
		}
		UE_LOG(LogEOK, Warning, TEXT("RegisterAntiCheatServer-> FOnlineSubsystemEOS is null"));
		return false;
	}
	UE_LOG(LogEOK, Warning, TEXT("RegisterAntiCheatServer-> IOnlineSubsystem is null"));
	return false;
}
...
bool UAntiCheatServer::UnregisterAntiCheatServer()
{
	if (IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
		{
			if(!EOSRef->AntiCheatServerHandle)
			{
				UE_LOG(LogEOK, Warning, TEXT("UnregisterAntiCheatServer-> AntiCheatServerHandle is null"));
				return false;
			}
			{
			...
			}
			{
				...
				if (Result == EOS_EResult::EOS_Success)
				{
					UE_LOG(LogEOK, Log, TEXT("UnregisterAntiCheatServer-> Success"));
					return true;
				}
				UE_LOG(LogEOK, Log, TEXT("UnregisterAntiCheatServer-> %hs"), EOS_EResult_ToString(Result));
				return false;
			}
		}
		UE_LOG(LogEOK, Warning, TEXT("UnregisterAntiCheatServer-> FOnlineSubsystemEOS is null"));
		return false;
	}
	UE_LOG(LogEOK, Warning, TEXT("UnregisterAntiCheatServer-> IOnlineSubsystem is null"));
	return false;
}
...
bool UAntiCheatServer::RegisterClientForAntiCheat(FString ClientProductID, APlayerController* ControllerRef, TEnumAsByte<EUserPlatform> UserPlatform, TEnumAsByte<EEOS_ClientType> ClientType)
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
		{
			if(!EOSRef->AntiCheatServerHandle)
			{
				UE_LOG(LogEOK, Warning, TEXT("RegisterClientForAntiCheat-> AntiCheatServerHandle is null"));
				return false;
			}
			...
			if(Result == EOS_EResult::EOS_Success)
			{
				UE_LOG(LogEOK, Log, TEXT("RegisterClientForAntiCheat-> Success"));
				return true;
			}
			UE_LOG(LogEOK, Warning, TEXT("RegisterClientForAntiCheat-> %hs"), EOS_EResult_ToString(Result));
			return false;
		}
		UE_LOG(LogEOK, Warning, TEXT("RegisterClientForAntiCheat-> FOnlineSubsystemEOS is null"));
		return false;
	}
	UE_LOG(LogEOK, Warning, TEXT("RegisterClientForAntiCheat-> IOnlineSubsystem is null"));
	return false;
}
...
bool UAntiCheatServer::UnregisterClientFromAntiCheat(APlayerController* ControllerRef)
{
	if (IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
		{
			if(!EOSRef->AntiCheatServerHandle)
			{
				UE_LOG(LogEOK, Warning, TEXT("UnregisterClientFromAntiCheat-> AntiCheatServerHandle is null"));
				return false;
			}
			{
...
bool UAntiCheatServer::RecievedMessageFromClient(APlayerController* Controller,const TArray<uint8>& Message)
{
	if(	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get("EOK"))
	{
		if (FOnlineSubsystemEOS* EOSRef = static_cast<FOnlineSubsystemEOS*>(OnlineSub))
		{
			if(!EOSRef->AntiCheatServerHandle)
			{
				UE_LOG(LogEOK, Warning, TEXT("RecievedMessageFromClient-> AntiCheatServerHandle is null"));
				return false;
			}
			...
			if(Result == EOS_EResult::EOS_Success)
			{
				UE_LOG(LogEOK, Log, TEXT("RecievedMessageFromClient-> Success"));
				return true;
			}
			UE_LOG(LogEOK, Log, TEXT("RecievedMessageFromClient-> %hs"), EOS_EResult_ToString(Result));
			return false;
		}
		UE_LOG(LogEOK, Warning, TEXT("RecievedMessageFromClient-> FOnlineSubsystemEOS is null"));
		return false;
	}
	UE_LOG(LogEOK, Warning, TEXT("RecievedMessageFromClient-> IOnlineSubsystem is null"));
	return false;
}
...
void UAntiCheatServer::OnMessageToClientCb(const EOS_AntiCheatCommon_OnMessageToClientCallbackInfo* Data)
{
	if(!Data->ClientData)
	{
		UE_LOG(LogEOK,Verbose, TEXT("OnMessageToClientCb-> ClientData is null"));
		return;
	}	
	...
	{
		UE_LOG(LogEOK,Verbose, TEXT("OnMessageToClientCb-> AntiCheatServer is null"));
	}
}
...
void UAntiCheatServer::OnClientActionRequiredCb(const EOS_AntiCheatCommon_OnClientActionRequiredCallbackInfo* Data)
{
	if(!Data->ClientData)
	{
		UE_LOG(LogEOK,Verbose, TEXT("OnClientActionRequiredCb-> ClientData is null"));
		return;
	}	
	if(const UAntiCheatServer* AntiCheatServer = static_cast<UAntiCheatServer*>(Data->ClientData))
	{
	...
	{
		UE_LOG(LogEOK,Verbose, TEXT("OnClientActionRequiredCb-> AntiCheatServer is null"));
	}
}
