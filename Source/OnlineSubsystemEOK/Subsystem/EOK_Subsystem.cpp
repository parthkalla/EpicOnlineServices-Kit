...
UEOK_Subsystem::UEOK_Subsystem()
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		...
		{
			SessionPtrRef->AddOnSessionUserInviteAcceptedDelegate_Handle(FOnSessionUserInviteAcceptedDelegate::CreateUObject(this, &UEOK_Subsystem::FuncEK_OnSessionUserInviteAccepted));
		}
	}
}


void UEOK_Subsystem::Login(int32 LocalUserNum, FString ID, FString Token , FString Type, const FBP_Login_Callback& Result)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			...
			IdentityPointerRef->OnLoginCompleteDelegates->AddUObject(this,&UEOK_Subsystem::LoginCallback);
...
/*
* This is a C++ function definition for a method called "Login" in the "UEOK_Subsystem" class. It takes in one parameters: an integer "LocalUserNum" It also takes in a callback function "Result" of type "FBP_Login_Callback".
 ...
 */
void UEOK_Subsystem::LoginWithDeviceID(int32 LocalUserNum, FString DisplayName, FString DeviceName, const FBP_Login_Callback& Result)
{
	Login(LocalUserNum,DisplayName, DeviceName, "deviceid", Result);
}

void UEOK_Subsystem::LoginWithAccountPortal(int32 LocalUserNum, const FBP_Login_Callback& Result)
{
	Login(LocalUserNum,"", "", "accountportal", Result);
}

void UEOK_Subsystem::LoginWithSteam(int32 LocalUserNum, const FBP_Login_Callback& Result)
{
	Login(LocalUserNum,"", "", "steam", Result);
}

void UEOK_Subsystem::LoginWithPersistantAuth(int32 LocalUserNum, const FBP_Login_Callback& Result)
{
	Login(LocalUserNum,"", "", "persistentauth", Result);
}

void UEOK_Subsystem::LoginWithDeveloperTool(int32 LocalUserNum, FString LocalIP, FString Credential, const FBP_Login_Callback& Result)
{
	Login(LocalUserNum,LocalIP, Credential, "developer", Result);
}

void UEOK_Subsystem::LoginWithEpicLauncher(int32 LocalUserNum, const FBP_Login_Callback& Result)
{
	FString EGS_Token;
 ...
 */
void UEOK_Subsystem::Logout(int32 LocalUserNum, const FBP_Logout_Callback& Result)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			IdentityPointerRef->OnLogoutCompleteDelegates->AddUObject(this,&UEOK_Subsystem::LogoutCallback);
 ...
 */
FString UEOK_Subsystem::GetPlayerNickname(const int32 LocalUserNum)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
 ...
 */
bool UEOK_Subsystem::GetLoginStatus(const int32 LocalUserNum)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
...
void UEOK_Subsystem::CreateEOSSession(const FBP_CreateSession_Callback& Result,
	...
	int32 NumberOfPublicConnections, ERegionInfo Region)
{
	...
	if(IOnlineSubsystem *SubsystemRef = Online::GetSubsystem(this->GetWorld()))
	{
		if(IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
		{
			...
			SessionPtrRef->OnCreateSessionCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnCreateSessionCompleted);
			SessionPtrRef->CreateSession(0,*SessionName,SessionCreationInfo);
		}
	}
}

void UEOK_Subsystem::CreateEOSLobby(const FBP_CreateLobby_Callback& Result, TMap<FString, FString> Custom_Settings,
	...
	bool bAllowJoinInProgress, bool bIsLan, int32 NumberOfPublicConnections, int32 NumberOfPrivateConnections)
{
	...
	if(IOnlineSubsystem *SubsystemRef = Online::GetSubsystem(this->GetWorld()))
	{
		if(IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
		{
			...
			SessionPtrRef->OnCreateSessionCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnCreateLobbyCompleted);
			SessionPtrRef->CreateSession(0,*SessionName,SessionCreationInfo);
		}
	}
}

void UEOK_Subsystem::FindEOSSession(const FBP_FindSession_Callback& Result, TMap<FString, FString> Search_Settings,
	EMatchType MatchType, ERegionInfo RegionToSearch)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
		{
			...
			SessionPtrRef->OnFindSessionsCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnFindSessionCompleted);
...
void UEOK_Subsystem::DestroyEosSession(const FBP_DestroySession_Callback& Result, FName SessionName)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
		{
			SessionPtrRef->OnDestroySessionCompleteDelegates.AddUObject(this,&UEOK_Subsystem::OnDestroySessionCompleted);
...
void UEOK_Subsystem::JoinEosSession(const FBP_JoinSession_Callback& Result, FName SessionName,
	...
	if(SessionResult.OnlineResult.IsSessionInfoValid())
	{
		if(SubsystemRef)
		{
			if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
			{
				...
				SessionPtrRef->OnJoinSessionCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnJoinSessionCompleted);
...
FEOKUniqueNetId UEOK_Subsystem::GetUserUniqueID() const
{
	if(const TSharedPtr<const FUniqueNetId> EOK_NetID = UGameplayStatics::GetGameInstance(GetWorld())->GetFirstGamePlayer()->GetPreferredUniqueNetId().GetUniqueNetId())
	{
...
FString UEOK_Subsystem::GetProductUserID(const FEOKUniqueNetId& UniqueNetId)
{
...
FString UEOK_Subsystem::GetEpicID(const FEOKUniqueNetId& UniqueNetId)
{
...
void UEOK_Subsystem::UnRegisterPlayer(FName SessionName)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
void UEOK_Subsystem::RegisterPlayer(FName SessionName,  bool bWasInvited)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
void UEOK_Subsystem::StartSession(FName SessionName)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
void UEOK_Subsystem::EndSession(FName SessionName)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineSessionPtr SessionPtrRef = SubsystemRef->GetSessionInterface())
...
bool UEOK_Subsystem::ShowFriendUserInterface()
{
...
void UEOK_Subsystem::UpdateStats(const FBP_UpdateStat_Callback& Result, FString StatName, int32 Amount)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			if(const IOnlineStatsPtr StatsPointerRef = SubsystemRef->GetStatsInterface())
			{
				...
				StatsPointerRef->UpdateStats(IdentityPointerRef->GetUniquePlayerId(0).ToSharedRef(),StatArray,FOnlineStatsUpdateStatsComplete::CreateUObject(this, &UEOK_Subsystem::OnUpdateStatsCompleted));
			}
		}
	}
}

void UEOK_Subsystem::GetStats(const FBP_GetStats_Callback& Result, TArray<FString> StatName)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			if(const IOnlineStatsPtr StatsPointerRef = SubsystemRef->GetStatsInterface())
			{
				...
				StatsPointerRef->QueryStats(IdentityPointerRef->GetUniquePlayerId(0).ToSharedRef(),Usersvar,StatName,FOnlineStatsQueryUsersStatsComplete::CreateUObject(this, &UEOK_Subsystem::OnGetStatsCompleted));
			}
		}
	}
}
...
void UEOK_Subsystem::PurchaseItem(const FBP_PurchaseOffer_Callback& Result, FString ItemID)
{
...
void UEOK_Subsystem::QueryOffers(const FBP_GetOffers_Callback& Result)
{
...
void UEOK_Subsystem::GetOwnedItems(const FBP_GetOwnedItems_Callback& Result)
{
...
FString UEOK_Subsystem::GenerateSessionCode(int32 CodeLength) const
{
	FString SessionCode;
...
bool UEOK_Subsystem::OnHostMigrated(const FBP_HostMigration_Callback& Result)
{
...
void UEOK_Subsystem::SetPlayerData(const FBP_WriteFile_Callback& Result, FString FileName, USaveGame* SavedGame)
{
			...
			if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get() )
			{
				if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
				{
					if(const IOnlineUserCloudPtr CloudPointerRef = SubsystemRef->GetUserCloudInterface())
					{
						...
						CloudPointerRef->OnWriteUserFileCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnWriteFileComplete);
...
void UEOK_Subsystem::GetPlayerData(const FBP_GetFile_Callback& Result, FString FileName)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			if(const IOnlineUserCloudPtr CloudPointerRef = SubsystemRef->GetUserCloudInterface())
			{
				...
				CloudPointerRef->OnReadUserFileCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnGetFileComplete);
...
void UEOK_Subsystem::EnumerateTitleFiles(const FBP_TitleFileList_Callback& Result)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			...
			{
				TitleFilePtr->OnEnumerateFilesCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnTitleFileListComplete);
...
TArray<FFileListStruct> UEOK_Subsystem::GetTitleFileList()
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
...
void UEOK_Subsystem::GetTitleFile(const FBP_GetTitleFile_Callback& Result,FString FileName)
{
	...
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
		{
			if(const IOnlineTitleFilePtr TitleFilePtr = SubsystemRef->GetTitleFileInterface())
			{
				TitleFilePtr->OnReadFileCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnTitleFileComplete);
...
TArray<uint8> UEOK_Subsystem::GetTitleFileContent(FString FileName)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
		if(const IOnlineIdentityPtr IdentityPointerRef = SubsystemRef->GetIdentityInterface())
...
void UEOK_Subsystem::GetLeaderboard(const FBP_GetFile_Callback& Result, FName LeaderboardName, int32 Rank, int32 Range)
{
	if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
	{
...
#endif
				Leaderboards->AddOnLeaderboardReadCompleteDelegate_Handle(FOnLeaderboardReadComplete::FDelegate::CreateUObject(this,&UEOK_Subsystem::OnLeaderboardListCompleted));
...
void UEOK_Subsystem::ConnectEosAndPlayFab(const FBP_ConnectEOSAndPlayFab_Callback& Result)
{
	...
	if(GetLoginStatus(0) && !GetPlayerNickname(0).IsEmpty())
	{
		...
		PlayFabClientAPI->LoginWithCustomID(CustomIDRequest,PlayFab::UPlayFabClientAPI::FLoginWithCustomIDDelegate::CreateUObject(this, &UEOK_Subsystem::OnLoginWithEpicIDPFSuccess),
		PlayFab::FPlayFabErrorDelegate::CreateUObject(this, &UEOK_Subsystem::OnLoginWithEpicIDPFFailure));
		#else
...
void UEOK_Subsystem::OnCreateLobbyCompleted(FName SessionName, bool bWasSuccessful) const
{
	if(bWasSuccessful)
	{
		if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
...
void UEOK_Subsystem::OnFindSessionCompleted(bool bWasSuccess) const
{
			...
			if (SessionSearch->SearchResults.Num() > 0)
			{
				for (int32 SearchIdx = 0; SearchIdx < SessionSearch->SearchResults.Num(); SearchIdx++)
				{
					...
					while (It)
					{
						...
						LocalArraySettings.Add(*SettingName.ToString(), FEOKAttribute(SettingValueString));
						++It;
					}
...
void UEOK_Subsystem::OnJoinSessionCompleted(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
void UEOK_Subsystem::OnDestroySessionCompleted(FName SessionName, bool bWasSuccess) const
{
	DestroySession_CallbackBP.ExecuteIfBound(bWasSuccess);
}

void UEOK_Subsystem::OnUpdateStatsCompleted(const FOnlineError& Result) const
{
...
void UEOK_Subsystem::OnGetStatsCompleted(const FOnlineError& ResultState,
	const TArray<TSharedRef<const FOnlineStatsUserStats>>& UsersStatsResult) const
{
	...
	{
		GetStats_CallbackBP.ExecuteIfBound(false, TArray<FEOK_Stats>());
		UE_LOG(LogTemp,Warning,TEXT("Getting stats failed with error - %s"), *ResultState.ToLogString());
	}
}

void UEOK_Subsystem::OnWriteFileComplete(bool bSuccess, const FUniqueNetId& UserID, const FString& FileName) const
{
	WriteFile_CallbackBP.ExecuteIfBound(true);
}

void UEOK_Subsystem::OnGetFileComplete(bool bSuccess, const FUniqueNetId& UserID, const FString& FileName) const
{
	if(bSuccess)
	{
		if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
...
void UEOK_Subsystem::OnTitleFileListComplete(bool bSuccess, const FString& Error) const
{
	TitleFileList_CallbackBP.ExecuteIfBound(bSuccess, Error);
}

void UEOK_Subsystem::OnTitleFileComplete(bool bSuccess, const FString& FileName) const
{
	GetTitleFile_CallbackBP.ExecuteIfBound(bSuccess);
}

void UEOK_Subsystem::OnLeaderboardListCompleted(bool bWasSuccess) const
{
}

void UEOK_Subsystem::OnFriendInviteAcceptedDestroySession(FName Name, bool bArg)
{
}

void UEOK_Subsystem::OnSessionUserInviteAccepted12(const bool bWasSuccessful, const int32 ControllerId,
	...
	if (bWasSuccessful)
	{
		...
		if (const IOnlineSessionPtr SessionInt = Online::GetSessionInterface(GetWorld()))
		{
			SessionInt->DestroySession(NAME_GameSession, FOnDestroySessionCompleteDelegate::CreateLambda(
				...
				{
					SessionInt->OnJoinSessionCompleteDelegates.AddUObject(this, &UEOK_Subsystem::OnJoinSessionCompleted);
...
void UEOK_Subsystem::FuncEK_OnSessionUserInviteAccepted(bool bArg, int I, TSharedPtr<const FUniqueNetId> UniqueNetId,
...
//Callback Functions
void UEOK_Subsystem::LoginCallback(int32 LocalUserNum, bool bWasSuccess, const FUniqueNetId& UserId,
                                       const FString& Error) const
{
	LoginCallBackBP.ExecuteIfBound(bWasSuccess,Error);
}

void UEOK_Subsystem::LogoutCallback(int32 LocalUserNum, bool bWasSuccess) const
{
	LogoutCallbackBP.ExecuteIfBound(bWasSuccess);
}

void UEOK_Subsystem::OnCreateSessionCompleted(FName SessionName, bool bWasSuccessful) const
{
	if(bWasSuccessful)
	{
		if(const IOnlineSubsystem *SubsystemRef = IOnlineSubsystem::Get())
