...
#define LOCTEXT_NAMESPACE "FEOKEditorModule"

DEFINE_LOG_CATEGORY(LogEOKEditor);
...
void FEOKEditorModule::StartupModule()
{
    FEosIconStyle::Initialize();
    // Register a function to be called when the menu system is initialized
    UToolMenus::RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(
        this, &FEOKEditorModule::RegisterMenuExtensions));
}

void FEOKEditorModule::ShutdownModule()
{
...
void FEOKEditorModule::BuildProcessCompleted(int I, bool bArg)
{
...
void FEOKEditorModule::HandleProcessCanceled()
{
...
void FEOKEditorModule::HandleProcessOutput(const FString& String)
{
}

void FEOKEditorModule::RegisterMenuExtensions()
{
    ...
    FToolMenuSection &Section = ToolbarMenu->AddSection("EOKSection", LOCTEXT("EOKSection", "EOK"), FToolMenuInsert("File", EToolMenuInsertType::After));
    auto ComboButton = FToolMenuEntry::InitComboButton(
        "MyCustomDropdownName",
        ComboButtonAction,
        FOnGetContent::CreateRaw(this, &FEOKEditorModule::GenerateMenuContent),
        LOCTEXT("MyCustomDropdown", "Epic Online Services-Kit V2"),
        LOCTEXT("MyCustomDropdown_Tooltip", "Epic Online Services-Kit V2 Menu"),
#if ENGINE_MAJOR_VERSION == 5
...
void FEOKEditorModule::OpenDevPortal()
{
    UEOKSettings* EOKSettings = GetMutableDefault<UEOKSettings>();
    FString ProductName = EOKSettings->ProductName;
    FString OrganizationId = EOKSettings->OrganizationName;
...
void FEOKEditorModule::OpenDevTool()
{
...
#if PLATFORM_WINDOWS
    FString MainModulePath = FPaths::Combine(*PluginRoot, TEXT("Source/ThirdParty/EOKSDK/Tools/EOS_DevAuthTool-win32-x64-1.2.1"));
    DevToolPath = FPaths::Combine(*MainModulePath, TEXT("EOS_DevAuthTool.exe"));
    if (!FPaths::FileExists(DevToolPath))
    {
        UE_LOG(LogEOKEditor, Error, TEXT("DevTool not found at %s"), *DevToolPath);
...
#elif PLATFORM_MAC
    DevToolPath = FPaths::Combine(*PluginRoot, TEXT("Source/ThirdParty/EOKSDK/Tools/EOS_DevAuthTool-darwin-x64-1.2.1/EOS_DevAuthTool.app"));
#else
    UE_LOG(LogEOKEditor, Error, TEXT("Unsupported platform"));
    ...
    UE_LOG(LogEOKEditor, Log, TEXT("DevToolPath: %s | Length: %d"), *DevToolPath, DevToolPath.Len());
...
void FEOKEditorModule::OpenRedistributableInstallerTool()
{
    ...
    if (DesktopPlatform)
    {
        ...
        if (bFileSelected && OutFileNames.Num() > 0)
        {
            ...
            IPluginManager& PluginManager = IPluginManager::Get();
            TSharedPtr<IPlugin> Plugin = PluginManager.FindPlugin(TEXT("EpicOnlineServices-Kit V2"));
            if (!Plugin.IsValid())
            {
                UE_LOG(LogEOKEditor, Error, TEXT("EpicOnlineServices-Kit V2 plugin not found"));
                return;
            ...
            FString EOSBootstrapperToolPath = PluginDir / TEXT("Source/ThirdParty/EOKSDK/Tools/EOSBootstrapperTool.exe");
 ...
 TSharedRef<SWidget> FEOKEditorModule::GenerateMenuContent()
{
...
#endif
        FUIAction(FExecuteAction::CreateRaw(this, &FEOKEditorModule::OpenDevPortal))
    );
...
#endif
    FUIAction(FExecuteAction::CreateRaw(this, &FEOKEditorModule::OpenDevTool))
);
    ...
    MenuBuilder.AddMenuEntry(
        ...
        FSlateIcon(FEditorStyle::GetStyleSetName(), "ContentBrowser.ImportIcon"),
#endif
        FUIAction(FExecuteAction::CreateRaw(this, &FEOKEditorModule::OpenRedistributableInstallerTool))
        );
...
FSlateIcon(FAppStyle::GetAppStyleSetName(), "PlayWorld.RepeatLastLaunch"),
...
#endif
        FUIAction(FExecuteAction::CreateRaw(this, &FEOKEditorModule::OnPackageAndDeploySelected))
    );
    ...
    MenuBuilder.AddMenuEntry(
        ...
        {
            UEOKSettings* Settings = GetMutableDefault<UEOKSettings>();
            ...
            if (auto Profile = LauncherProfileManagerRef->FindProfile("EOKProfile"))
            {
            ...
            LauncherProfilePtr->SetName("EOKProfile");
    ...
    MenuBuilder.AddMenuEntry(
        ...
        FUIAction(FExecuteAction::CreateLambda([]
        {
            FModuleManager::LoadModuleChecked<ISettingsModule>("Settings").ShowViewer("Project", "Game", "Online Subsystem EOK");
        }))
        );
    ...
    FUIAction(FExecuteAction::CreateLambda([]
    {
        UKismetSystemLibrary::LaunchURL("EOK.betide.studio");
    }))
);
...
void FEOKEditorModule::OnPackageAndDeploySelected()
{
#if PLATFORM_WINDOWS
    UEOKSettings* Settings = GetMutableDefault<UEOKSettings>();
    ...
    ILauncherServicesModule& LauncherServicesModule = FModuleManager::LoadModuleChecked<ILauncherServicesModule>(TEXT("LauncherServices"));
    ILauncherProfileManagerRef LauncherProfileManagerRef = LauncherServicesModule.GetProfileManager();
    auto LauncherProfilePtr = LauncherProfileManagerRef->FindProfile("EOKProfile");
    ...
    GEditor->LauncherWorker->OnOutputReceived().AddLambda([](const FString& String) {
        UE_LOG(LogEOKEditor, Log, TEXT("%s"), *String);
    });
    ...
    GEditor->LauncherWorker->OnCompleted().AddLambda([this, Settings](bool bArg, double X, int I)
{
    ...
    {
        IPluginManager& PluginManager = IPluginManager::Get();
        FString PluginDir = PluginManager.FindPlugin("EpicOnlineServicesKitV2")->GetBaseDir();
        FString BuildPatchToolDir = PluginDir / TEXT("Source/ThirdParty/EOKSDK/Tools/BuildPatchTool_1.6.0/Engine/Binaries/Win64");
        ...
        UE_LOG(LogEOKEditor, Log, TEXT("Build Patch Tool Parameters: %s"), *ParametersForBuildPatchTool);
        InteractiveProcess = MakeShareable(new FInteractiveProcess(*BuildPatchToolExe, ParametersForBuildPatchTool, false, true));
        InteractiveProcess->OnCompleted().BindRaw(this, &FEOKEditorModule::BuildProcessCompleted);
        InteractiveProcess->OnCanceled().BindRaw(this, &FEOKEditorModule::HandleProcessCanceled);
        InteractiveProcess->OnOutput().BindRaw(this, &FEOKEditorModule::HandleProcessOutput);
...
IMPLEMENT_MODULE(FEOKEditorModule, EOKEditor)
