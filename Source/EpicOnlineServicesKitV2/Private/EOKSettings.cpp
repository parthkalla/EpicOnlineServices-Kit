...
#include UE_INLINE_GENERATED_CPP_BY_NAME(EOKSettings)
#endif
...
#define INI_SECTION TEXT("/Script/EpicOnlineServicesKitV2.EOKSettings")
...
FEOSSettings UEOKSettings::GetSettings()
{
	if (UObjectInitialized())
	{
		return UEOKSettings::AutoGetSettings();
	}

	return UEOKSettings::ManualGetSettings();
}

FEOSSettings UEOKSettings::AutoGetSettings()
{
	return GetDefault<UEOKSettings>()->ToNative();
}

const FEOSSettings& UEOKSettings::ManualGetSettings()
{
...
FEOSSettings UEOKSettings::ToNative() const
{
	FEOSSettings Native;
...
#if WITH_EDITOR
EAppReturnType::Type UEOKSettings::ShowRestartWarning(const FText& Title)
{
...
bool UEOKSettings::GetSettingsForArtifact(const FString& ArtifactName, FEOSArtifactSettings& OutSettings)
{
	if (UObjectInitialized())
	{
		return UEOKSettings::AutoGetSettingsForArtifact(ArtifactName, OutSettings);
	}
	return UEOKSettings::ManualGetSettingsForArtifact(ArtifactName, OutSettings);
}

bool UEOKSettings::ManualGetSettingsForArtifact(const FString& ArtifactName, FEOSArtifactSettings& OutSettings)
{
	...
	{
    	UE_LOG(LogEpicOnlineServicesKitV2, Log, TEXT("Overriding DeploymentId with %s"), *DeploymentIdOverride);
	}
    if(FParse::Value(FCommandLine::Get(), TEXT("epicsandboxid="), SandboxIdOverride))
    {
	    UE_LOG(LogEpicOnlineServicesKitV2, Log, TEXT("Overriding SandboxId with %s"), *SandboxIdOverride);
    }
...
bool UEOKSettings::AutoGetSettingsForArtifact(const FString& ArtifactName, FEOSArtifactSettings& OutSettings)
{
	const UEOKSettings* This = GetDefault<UEOKSettings>();
	FString DeploymentIdOverride;
	FString SandboxIdOverride;
	if(FParse::Value(FCommandLine::Get(), TEXT("epicdeploymentid="), DeploymentIdOverride))
	{
	...
	if(!ArtifactNameOverride.IsEmpty())
	{
		UE_LOG(LogEpicOnlineServicesKitV2, Error, TEXT("Failed to find artifact settings for %s"), *ArtifactNameOverride);
	}
	else
	{
		UE_LOG(LogEpicOnlineServicesKitV2, Error, TEXT("Failed to find artifact settings or the default artifact because the name was empty. Please check project settings under Epic Online Services-Kit V2"));
	}
	return false;
}

#if WITH_EDITOR
void UEOKSettings::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if(PropertyChangedEvent.Property->GetFName()==FName(TEXT("bAutomaticallySetupEOK")))
	{
		if(bAutomaticallySetupEOK)
		{
