 ...
 */
UCLASS()
class EOSKITSHARED_API UEOSKitSimpleSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
private:
	...
	UFUNCTION()
	void HandleLoginResult(bool bWasSuccess, const FString& Error);

	UFUNCTION()
	void HandleLogoutResult(bool bWasSuccess);

	UFUNCTION()
	void HandleCreateSessionResult(bool bWasSuccess, const FName& SessionName);

	UFUNCTION()
	void HandleCreateLobbyResult(bool bWasSuccess, const FName& SessionName);

	UFUNCTION()
	void HandleDestroySessionResult(bool bWasSuccess);

	// Stored callbacks for helper functions
	FEOSKit_Login_Callback StoredLoginCallback;
	FEOSKit_Logout_Callback StoredLogoutCallback;
