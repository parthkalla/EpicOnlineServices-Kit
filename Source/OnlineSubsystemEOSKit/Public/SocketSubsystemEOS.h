 ...
 * Based on EIK's FSocketSubsystemEIK implementation
 */
class ONLINESUBSYSTEMEOSKIT_API FSocketSubsystemEOS : public ISocketSubsystem
{
public:
	FSocketSubsystemEOS(void* InPlatformHandle, ISocketSubsystemEOSUtilsPtr InUtils);
	...
	/**
	 * Returns the FSocketSubsystemEOS instance corresponding with the UWorld* parameter.
	...
	/**
	 * Create a unique socket (for compatibility with EIK)
	 */
	TUniquePtr<FSocket> CreateUniqueSocket(const FName& SocketType, const FString& SocketDescription, const FName& ProtocolType);

	/**
	 * Bind socket to next available channel (for compatibility with EIK)
	 * For EOS, this just binds the socket to the address
	 */
	bool BindNextPort(FSocket* Socket, const FInternetAddrEOS& Address, int32 PortCount, int32 PortIncrement);
