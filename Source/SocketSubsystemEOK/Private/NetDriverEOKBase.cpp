...
#include UE_INLINE_GENERATED_CPP_BY_NAME(NetDriverEOKBase)
#endif

UNetDriverEOKBase::UNetDriverEOKBase(const FObjectInitializer& ObjectInitializer)
	: UIpNetDriver(ObjectInitializer)
{
	...
	bool bUnused;
	if (GConfig->GetBool(TEXT("/Script/SocketSubsystemEOK.NetDriverEOKBase"), TEXT("bIsUsingP2PSockets"), bUnused, GEngineIni))
	{
		UE_LOG(LogTemp, Warning, TEXT("EOK NetDriver: bIsUsingP2PSockets is deprecated, please remove any related config values"));
	}
#else
	// For UE 5.5 and below, initialize bIsUsingP2PSockets from config
	if (!GConfig->GetBool(TEXT("/Script/SocketSubsystemEOK.NetDriverEOKBase"), TEXT("bIsUsingP2PSockets"), bIsUsingP2PSockets, GEngineIni))
	{
...
bool UNetDriverEOKBase::IsAvailable() const
{
	// Use passthrough sockets if we are a dedicated server
	if (IsRunningDedicatedServer())
	{
		UE_LOG(LogTemp, Verbose, TEXT("NetDriverEOKBase::IsAvailable: Returning false (dedicated server)"));
		return false;
	}

	if (ISocketSubsystem* SocketSubsystem = ISocketSubsystem::Get(EOS_SOCKETSUBSYSTEM))
	{
		UE_LOG(LogTemp, Verbose, TEXT("NetDriverEOKBase::IsAvailable: Returning true (EOS socket subsystem available)"));
		return true;
	}

	UE_LOG(LogTemp, Warning, TEXT("NetDriverEOKBase::IsAvailable: Returning false (EOS socket subsystem not available)"));
	return false;
}

bool UNetDriverEOKBase::InitBase(bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error)
{
	// For client connections, check if this is an EOS URL early
	// When URL is parsed, "EOS:..." becomes protocol="EOS", host="PUID:SocketName:Channel"
	if (bInitAsClient)
	{
		bool bIsEOSURL = URL.Protocol == EOS_CONNECTION_URL_PREFIX || 
		                 URL.Host.StartsWith(EOS_CONNECTION_URL_PREFIX, ESearchCase::IgnoreCase);
...
#endif
		{
			// Not an EOS URL or EOS not available, use passthrough
			bIsPassthrough = true;
			UE_LOG(LogTemp, Verbose, TEXT("Running as pass-through (not EOS URL or EOS unavailable)"));
			return Super::InitBase(bInitAsClient, InNotify, URL, bReuseAddressAndPort, Error);
		}
		// Clear passthrough flag if we're handling EOS URL
		bIsPassthrough = false;
	}
	...
	FSocketSubsystemEOK* const SocketSubsystem = static_cast<FSocketSubsystemEOK*>(GetSocketSubsystem());
	if (!SocketSubsystem)
	{
...
bool UNetDriverEOKBase::InitConnect(FNetworkNotify* InNotify, const FURL& ConnectURL, FString& Error)
{
	// Check if this is an EOS URL by checking the protocol or if the host starts with EOS
	// When URL is parsed, "EOS:..." becomes protocol="EOS", host="PUID:SocketName:Channel"
	bool bIsEOSURL = ConnectURL.Protocol == EOS_CONNECTION_URL_PREFIX || 
	                 ConnectURL.Host.StartsWith(EOS_CONNECTION_URL_PREFIX, ESearchCase::IgnoreCase);
	bool bIsAvailableResult = IsAvailable();
	...
	// If protocol is EOS, reconstruct the full connection string from host
	// Otherwise, use the host as-is (for backwards compatibility)
	FString ConnectionString = ConnectURL.Host;
	if (ConnectURL.Protocol == EOS_CONNECTION_URL_PREFIX)
	{
		// Reconstruct: EOS:Host (where Host already contains PUID:SocketName:Channel)
		ConnectionString = FString::Printf(TEXT("%s:%s"), EOS_CONNECTION_URL_PREFIX, *ConnectURL.Host);
	}
	
	RemoteHost->SetIp(*ConnectionString, bIsValid);
	...
	UNetConnectionEOK* Connection = NewObject<UNetConnectionEOK>(NetConnectionClass);
	check(Connection);
...
bool UNetDriverEOKBase::InitListen(FNetworkNotify* InNotify, FURL& LocalURL, bool bReuseAddressAndPort, FString& Error)
{
...
ISocketSubsystem* UNetDriverEOKBase::GetSocketSubsystem()
{
	if (bIsPassthrough)
	{
	...
	FSocketSubsystemEOK* DefaultSocketSubsystem = static_cast<FSocketSubsystemEOK*>(ISocketSubsystem::Get(EOS_SOCKETSUBSYSTEM));
	return DefaultSocketSubsystem->GetSocketSubsystemForWorld(CurrentWorld);
}

void UNetDriverEOKBase::Shutdown()
{
	Super::Shutdown();
	...
	if (!bIsPassthrough)
	{
		if(ServerConnection)
		{
			if (UNetConnectionEOK* const EOSServerConnection = Cast<UNetConnectionEOK>(ServerConnection))
			{
		...
		for (UNetConnection* Client : ClientConnections)
		{
			if(Client)
			{
				if (UNetConnectionEOK* const EOSClient = Cast<UNetConnectionEOK>(Client))
				{
...
int UNetDriverEOKBase::GetClientPort()
{
	if (bIsPassthrough)
	{
...
bool UNetDriverEOKBase::IsBeaconDriver() const
{
...
UWorld* UNetDriverEOKBase::FindWorld() const
{
