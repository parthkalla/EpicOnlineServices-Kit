...
FSocketEOS::FSocketEOS(FSocketSubsystemEOK& InSocketSubsystem, const FString& InSocketDescription)
...
{
	// Close() now handles channel unbinding, so we don't need to unbind here again
	// Just clear the local address
	Close();

	if (LocalAddress.IsValid())
	{
		// Channel was already unbound in Close(), just clear the address
...
bool FSocketEOS::Close()
{
	...
	if (LocalAddress.IsValid())
	{
		...
		// Unbind the channel when closing the socket to allow immediate reconnection
		// This is especially important during travel when connections are closed and recreated
		SocketSubsystem.UnbindChannel(LocalAddress);
		UE_LOG(LogSocketSubsystemEOS, Verbose, TEXT("Unbound channel for socket during close: %s"), *LocalAddress.ToString(true));
	}
#endif
	return true;
}
